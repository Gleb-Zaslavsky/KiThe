use crate::Kinetics::solid_state_kinetics_IVP::{KineticModelNames, KineticModelIVP};
...
fn input_model_params(count: usize) -> Result<Vec<f64>, String> {
    ...
    for i in 0..count {
        ...
        let param: f64 = input.trim().parse()
...
fn input_parameters() -> Result<(f64, f64, f64, f64, f64), String> {
    ...
    let mut input = String::new();
    io::stdin().read_line(&mut input).unwrap();
    let t_final: f64 = input.trim().parse()
