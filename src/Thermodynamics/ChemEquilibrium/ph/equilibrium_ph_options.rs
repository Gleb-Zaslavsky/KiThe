...
/// Temperature-start policy for the coupled `[ln(n), theta_T]` formulation.
///
/// Every candidate is a fresh nonlinear transaction built from the same
/// caller-approved composition. Failed backend iterates are never promoted to
/// a later seed.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum PhMonolithicSeedPolicy {
    /// Execute exactly the caller-provided temperature seed.
    InitialOnly,
    /// On a retryable numerical failure, probe the bounded interval midpoint
    /// and its quarter points in deterministic order.
    DeterministicBoundedRecovery,
}

impl Default for PhMonolithicSeedPolicy {
    fn default() -> Self {
        Self::DeterministicBoundedRecovery
    }
}

/// One physical-temperature attempt in a monolithic P,H seed transaction.
#[derive(Debug, Clone, PartialEq)]
pub struct PhMonolithicSeedAttemptReport {
    pub(crate) temperature_seed_k: f64,
    pub(crate) accepted: bool,
    pub(crate) failure_kind: Option<ReactionExtentErrorKind>,
    pub(crate) error: Option<String>,
    pub(crate) started_backend_attempts: usize,
    pub(crate) nonlinear_iterations: usize,
}

impl PhMonolithicSeedAttemptReport {
    ...
    /// Returns `true` when this start produced the published candidate.
    pub fn accepted(&self) -> bool {
        self.accepted
    }

    /// Machine-readable failure class, absent for the accepted start.
    pub fn failure_kind(&self) -> Option<ReactionExtentErrorKind> {
        self.failure_kind
    }

    /// Human-readable failure detail, absent for the accepted start.
    pub fn error(&self) -> Option<&str> {
        self.error.as_deref()
    }

    /// Number of nonlinear backends started for this temperature seed.
    pub fn started_backend_attempts(&self) -> usize {
        self.started_backend_attempts
    }

    /// Aggregate backend iterations consumed by this temperature seed.
    pub fn nonlinear_iterations(&self) -> usize {
        self.nonlinear_iterations
    }
}

/// Immutable evidence for deterministic monolithic temperature recovery.
#[derive(Debug, Clone, PartialEq)]
pub struct PhMonolithicSeedRecoveryReport {
    pub(crate) policy: PhMonolithicSeedPolicy,
    pub(crate) attempts: Vec<PhMonolithicSeedAttemptReport>,
    pub(crate) selected_attempt: usize,
}

impl PhMonolithicSeedRecoveryReport {
    /// Policy that generated the ordered temperature starts.
    pub fn policy(&self) -> PhMonolithicSeedPolicy {
        self.policy
    }

    /// Ordered attempts, beginning with the caller-approved temperature.
    pub fn attempts(&self) -> &[PhMonolithicSeedAttemptReport] {
        &self.attempts
    }

    /// Index of the first accepted attempt.
    pub fn selected_attempt(&self) -> usize {
        self.selected_attempt
    }

    /// Returns `true` when the caller seed failed and a bounded retry won.
    pub fn recovered_after_retry(&self) -> bool {
        self.selected_attempt > 0
    }

    /// Aggregate number of nonlinear backends started across all seeds.
    pub fn started_backend_attempts(&self) -> usize {
        self.attempts
            .iter()
            .map(PhMonolithicSeedAttemptReport::started_backend_attempts)
            .sum()
    }

    /// Aggregate backend iterations consumed across all seeds.
    pub fn nonlinear_iterations(&self) -> usize {
        self.attempts
            .iter()
            .map(PhMonolithicSeedAttemptReport::nonlinear_iterations)
            .sum()
    }
}
...
impl PhMonolithicOptions {
    ...
    pub const fn new(acceptance: PhAcceptanceOptions, seed_policy: PhMonolithicSeedPolicy) -> Self {
        Self {
            acceptance,
            seed_policy,
        }
    ...
    /// Deterministic temperature recovery policy used after a retryable
    /// monolithic failure.
    pub const fn seed_policy(self) -> PhMonolithicSeedPolicy {
        self.seed_policy
    }
}

impl Default for PhMonolithicOptions {
    fn default() -> Self {
        Self::new(
            PhAcceptanceOptions::default_values(),
            PhMonolithicSeedPolicy::default(),
        )
    }
}
...
mod tests {
    ...
    fn route_options_keep_acceptance_shared_but_do_not_mix_controls() {
        ...
        let monolithic = PhMonolithicOptions::new(
            acceptance,
