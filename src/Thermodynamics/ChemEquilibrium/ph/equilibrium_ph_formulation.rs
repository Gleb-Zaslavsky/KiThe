...
use crate::Thermodynamics::ChemEquilibrium::equilibrium_ph_options::PhMonolithicSeedPolicy;
...
impl PreparedPhFormulation {
    ...
    /// Builds the ordered, duplicate-free physical temperature starts for one
    /// monolithic transaction. Recovery points are defined by the declared
    /// thermochemistry interval, never by an external reference answer.
    pub(crate) fn temperature_seed_candidates(
        &self,
        requested_temperature: f64,
        policy: PhMonolithicSeedPolicy,
    ) -> Result<Vec<f64>, ReactionExtentError> {
        let initial = self
            .temperature_transform
            .interior_seed(requested_temperature)?;
        let mut seeds = vec![initial];
        if policy == PhMonolithicSeedPolicy::DeterministicBoundedRecovery {
            let lower = self.temperature_transform.bounds.lower();
            let span = self.temperature_transform.span;
            let duplicate_tolerance = (span * 1.0e-12).max(1.0e-12);
            for fraction in [0.5, 0.25, 0.75] {
                let candidate = lower + fraction * span;
                if seeds
                    .iter()
                    .all(|seed| (seed - candidate).abs() > duplicate_tolerance)
                {
...
mod tests {
    ...
    #[test]
    fn monolithic_temperature_recovery_seeds_are_bounded_ordered_and_deduplicated() {
        let formulation = prepared_two_species_ph();

        assert_eq!(
            formulation
                .temperature_seed_candidates(700.0, PhMonolithicSeedPolicy::InitialOnly)
                .unwrap(),
            vec![700.0]
        );
        assert_eq!(
            formulation
                .temperature_seed_candidates(
                    700.0,
                    PhMonolithicSeedPolicy::DeterministicBoundedRecovery,
                )
                .unwrap(),
            vec![700.0, 1_500.0, 1_000.0, 2_000.0]
        );
        assert_eq!(
            formulation
                .temperature_seed_candidates(
