...
use crate::Thermodynamics::ChemEquilibrium::equilibrium_ph_workflow::{
    ...
    recover_resolved_ph_after_numerical_failure, solve_resolved_ph,
};
...
pub struct PhRangePointReport {
    ...
    extensive_normalization_recovery: bool,
...
impl PhRangePointReport {
    ...
    /// Whether this point was published after normalized extensive recovery.
    pub fn used_extensive_normalization_recovery(&self) -> bool {
        self.extensive_normalization_recovery
    }
...
pub struct PhRangeSolveReport {
    ...
    formulation_builds: usize,
    formulation_reuses: usize,
    extensive_normalization_recoveries: usize,
...
impl PhRangeSolveReport {
    ...
    /// Number of accepted points recovered through normalized extensive
    /// coordinates. Recovery is point-local and never becomes continuation
    /// state in its own representation.
    pub fn extensive_normalization_recoveries(&self) -> usize {
        self.extensive_normalization_recoveries
    }
...
impl<'a> PhRangeRequest<'a> {
    ...
    pub fn solve(self) -> Result<PhRangeSolution, PhRangeError> {
        ...
        let mut extensive_normalization_recoveries = 0usize;
        let uses_prepared_fixed_layout = self.prototype.uses_fixed_declared_phases();
        let mut prepared_monolithic = if matches!(
            self.prototype.ph_solve_mode(),
            PhSolveMode::Monolithic | PhSolveMode::Auto
        ) && uses_prepared_fixed_layout
        {
            Some(
        ...
        let prepared_nested = if matches!(
            self.prototype.ph_solve_mode(),
        ...
        for (index, target) in self.targets.values().iter().copied().enumerate() {
            ...
            // Keep an untouched request for classified recovery. The chosen
            // prepared driver may consume the working request, while the
            // recovery must start from the same physical point transaction.
            let recovery_request = request.clone();
            let formulation_reused = prepared_monolithic.is_some() && index > 0;
            let solve_result = if let Some(state) = prepared_monolithic.as_mut() {
                match state.solve(&request, formulation_reused) {
                    Ok(solution) => Ok(solution),
                    Err(monolithic_error)
                        if self.prototype.ph_solve_mode() == PhSolveMode::Auto
                            && monolithic_error.is_retryable_formulation_failure() =>
                    {
                        let fallback_reason = PhFallbackReason::from_error(&monolithic_error);
                        let nested_request =
                            request.cloned_with_ph_solve_mode(PhSolveMode::NestedTemperature);
                        match prepared_nested
                            .as_ref()
                            .expect("fixed-layout Auto range must prepare nested recovery state")
                            .solve(nested_request)
                        {
                            Ok(solution) => Ok(solution.with_auto_fallback(fallback_reason)),
                            Err(nested_error) => Err(ReactionExtentError::PhAutoFallbackFailed {
                                monolithic: Box::new(monolithic_error),
                                nested: Box::new(nested_error),
                            }),
                        }
                    }
                    Err(error) => Err(error),
            ...
            let solve_result = match solve_result {
                Ok(solution) => Ok(solution),
                Err(error) => recover_resolved_ph_after_numerical_failure(recovery_request, error),
            };
            let mut solution = solve_result.map_err(|source| {
            ...
            let point_recovered = solution
                .equilibrium()
                .extensive_normalization_recovery()
                .is_some();
            extensive_normalization_recoveries += usize::from(point_recovered);
            ...
            points.push(PhRangePoint {
                report: PhRangePointReport {
                    ...
                    formulation_builds: solution.report().fixed_formulation_builds(),
                    formulation_reuses: solution.report().fixed_formulation_reuses(),
                    extensive_normalization_recovery: point_recovered,
        ...
        let report = PhRangeSolveReport {
            ...
            formulation_builds,
            formulation_reuses,
            extensive_normalization_recoveries,
