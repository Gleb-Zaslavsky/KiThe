...
use crate::Thermodynamics::ChemEquilibrium::equilibrium_ph_monolithic::{
    PreparedMonolithicPhRunner, solve_monolithic_active_set_candidate_with_seed_report,
};
#[cfg(test)]
...
pub use crate::Thermodynamics::ChemEquilibrium::equilibrium_ph_options::{
    ...
    PhMonolithicSeedPolicy, PhMonolithicSeedRecoveryReport, PhMonotonicityPolicy, PhNestedOptions,
};
pub use crate::Thermodynamics::ChemEquilibrium::equilibrium_ph_thermochemistry::{
    ...
    ThermochemistryStandardStatePressure,
};
...
///
/// The production default is `Auto`: it runs the coupled monolithic path with
/// its bounded seed recovery first, then uses the independently formulated
/// safeguarded nested route only after a classified numerical failure.
...
impl PhFallbackReason {
    ...
    pub(crate) fn from_error(error: &ReactionExtentError) -> Self {
        Self {
...
pub struct PhTemperatureSolveOptions {
    ...
    /// Initial-temperature recovery policy for the coupled monolithic route.
    pub monolithic_seed_policy: PhMonolithicSeedPolicy,
...
impl Default for PhTemperatureSolveOptions {
    fn default() -> Self {
        Self {
            ...
            monolithic_seed_policy: PhMonolithicSeedPolicy::default(),
...
impl PhTemperatureSolveOptions {
    /// Selects how the monolithic route recovers from a retryable failure at
    /// the caller-provided temperature seed.
    pub fn with_monolithic_seed_policy(mut self, policy: PhMonolithicSeedPolicy) -> Self {
        self.monolithic_seed_policy = policy;
        self
    }
    ...
    pub fn monolithic_options(&self) -> Result<PhMonolithicOptions, ReactionExtentError> {
        Ok(PhMonolithicOptions::new(
            self.acceptance_options()?,
            self.monolithic_seed_policy,
        ))
    }
...
/// Applies the route-independent work contract before a monolithic candidate
/// can cross the publication boundary.
fn enforce_monolithic_work_limits(
    ...
    if let Some(limit) = options.max_inner_backend_attempts {
        if backend_attempts > limit {
            return Err(ReactionExtentError::InvalidProblem {
                field: "inner_backend_budget",
                message: format!(
                    "monolithic P,H solve started {backend_attempts} backend attempt(s), exceeding the configured limit of {limit}"
                ),
            });
        }
    }
    if let Some(limit) = options.max_inner_nonlinear_iterations {
        if nonlinear_iterations > limit {
            return Err(ReactionExtentError::InvalidProblem {
                field: "inner_nonlinear_iteration_budget",
                message: format!(
                    "monolithic P,H solve used {nonlinear_iterations} nonlinear iteration(s), exceeding the configured limit of {limit}"
                ),
            });
        }
    }
    if let Some(limit) = options.max_phase_control_transitions {
        if phase_control_transitions > limit {
            return Err(ReactionExtentError::InvalidProblem {
                field: "phase_control_transition_budget",
                message: format!(
                    "monolithic P,H solve accepted {phase_control_transitions} phase transition(s), exceeding the configured limit of {limit}"
                ),
            });
        }
    }
    if let Some(limit) = options.max_wall_time {
        if started.elapsed() > limit {
            return Err(ReactionExtentError::InvalidProblem {
                field: "wall_time_budget",
                message: format!("monolithic P,H solve exceeded its wall-time budget of {limit:?}"),
            });
        }
    }
...
pub struct PhMonolithicEvidence {
    ...
    temperature_seed_report: Option<PhMonolithicSeedRecoveryReport>,
...
impl PhMonolithicEvidence {
    ...
    /// Ordered physical-temperature starts used by the coupled formulation.
    pub fn temperature_seed_report(&self) -> Option<&PhMonolithicSeedRecoveryReport> {
        self.temperature_seed_report.as_ref()
    }
...
impl FixedPressureEnthalpySolution {
    ...
    /// Marks a nested result as the accepted recovery of one rejected
    /// monolithic `Auto` attempt without fabricating scalar trial records.
    pub(crate) fn with_auto_fallback(mut self, reason: PhFallbackReason) -> Self {
    ...
    /// Maps an accepted normalized P,H result back to physical extensive
    /// units. The caller must supply a build report made from the original
    /// physical request at the accepted temperature; this prevents normalized
    /// element totals from leaking into the public provenance report.
    pub(crate) fn reconstruct_physical_extensive_boundary(
        mut self,
        normalization: ExtensiveNormalization,
        physical_build_report: crate::Thermodynamics::ChemEquilibrium::phase_equilibrium_problem::PhaseEquilibriumBuildReport,
    ) -> Result<Self, ReactionExtentError> {
        self.solution = self
            .solution
            .reconstruct_physical_extensive_boundary(normalization, physical_build_report)?;
        self.calculated_enthalpy = normalization
            .denormalize_total_enthalpy(
                crate::Thermodynamics::ChemEquilibrium::equilibrium_constraints::TotalEnthalpyJoules::new(
                    self.calculated_enthalpy,
                )?,
            )?
            .joules();
        self.target_enthalpy = normalization
            .denormalize_total_enthalpy(
                crate::Thermodynamics::ChemEquilibrium::equilibrium_constraints::TotalEnthalpyJoules::new(
                    self.target_enthalpy,
                )?,
            ...
            .joules();
        self.report.target_enthalpy = normalization
            .denormalize_total_enthalpy(
                crate::Thermodynamics::ChemEquilibrium::equilibrium_constraints::TotalEnthalpyJoules::new(
                    self.report.target_enthalpy,
                )?,
            )?
            .joules();
        self.report.enthalpy_scale_joules =
            normalization.denormalize_extensive_error(self.report.enthalpy_scale_joules)?;
        self.report.absolute_enthalpy_tolerance_joules = normalization
            .denormalize_extensive_error(self.report.absolute_enthalpy_tolerance_joules)?;
        Ok(self)
    }
}
...
pub(crate) struct PreparedPhContinuationState {
    ...
    cached_target_enthalpy: f64,
    cached_enthalpy_scale: f64,
}
...
impl PreparedNestedPhContinuationState {
    ...
    pub(crate) fn new(
        ...
        request.validate()?;
        if !matches!(
            request.ph_solve_mode,
            PhSolveMode::NestedTemperature | PhSolveMode::Auto
...
impl PreparedPhContinuationState {
    ...
    pub(crate) fn new(
        ...
        if !matches!(
            request.ph_solve_mode,
            PhSolveMode::Monolithic | PhSolveMode::Auto
        ) || !matches!(
            request.solve_mode,
            PhaseEquilibriumSolveMode::FixedDeclaredPhases
        ) {
        ...
        Ok(Self {
            ...
            thermochemistry,
            rst_problem,
            cached_target_enthalpy: target,
            cached_enthalpy_scale: scale.joules(),
        })
    }
    ...
    pub(crate) fn solve(
        ...
        // Retarget local copies. The prepared cache is committed only after
        // the candidate has passed the ordinary publication boundary; a
        // failed point must not become the seed/formulation of the next one.
        let formulation = self.formulation.retarget(prepared.clone(), target, scale)?;
        let options = request.temperature_options.clone();
        let monolithic_options = options.monolithic_options()?;
        ...
        // Delay the only mutable backend-cache write until all fallible
        // preparation and cancellation checks have completed. The error path
        // below then has one explicit restoration point.
        if let Some(rst_problem) = self.rst_problem.as_mut() {
            rst_problem.set_ph_parameters(target, scale.joules())?;
        }
        let outcome = match runner.solve_from_log_moles_and_temperature_seed(
            &log_seed,
            initial_temperature,
            ...
            Ok(outcome) => outcome,
            Err(error) => {
                // RST owns a mutable parameter buffer, so restore its last
                // committed values before returning a failed transaction.
                if let Some(rst_problem) = self.rst_problem.as_mut() {
                    rst_problem.set_ph_parameters(
                        self.cached_target_enthalpy,
                        self.cached_enthalpy_scale,
                    )?;
                }
                enforce_monolithic_work_limits(
                    &options,
                    started,
        ...
        enforce_monolithic_work_limits(
            &options,
            started,
            outcome.temperature_seed_report.started_backend_attempts(),
            outcome.temperature_seed_report.nonlinear_iterations(),
            0,
        )?;
        let accepted_conditions = request
            .constraint
            .conditions_at(outcome.snapshot.temperature)?;
        let accepted_solution = prepared.accepted_solution_at_conditions(
        ...
        let report = PhTemperatureSolveReport {
            ...
            inner_backend_attempts: outcome.temperature_seed_report.started_backend_attempts(),
            inner_nonlinear_iterations: outcome.temperature_seed_report.nonlinear_iterations(),
            ...
            monolithic_evidence: Some(PhMonolithicEvidence {
                ...
                temperature_seed_report: Some(outcome.temperature_seed_report.clone()),
        ...
        // Commit the retargeted structural state only after construction of a
        // complete accepted result succeeded.
        self.prepared.prepared = prepared;
        self.formulation = formulation;
        self.cached_target_enthalpy = target;
        self.cached_enthalpy_scale = scale.joules();
        Ok(result)
    }
}
...
/// canonical resolved-data request defaults to the auditable `Auto` path.
...
/// Recovers one rejected P,H point in an exactly equivalent normalized
/// extensive representation.
///
/// This entry point is intentionally separate from [`solve_resolved_ph`]: a
/// range driver calls it only after its prepared fast path has failed. The
/// normalized transaction is fresh, its accepted result is reconstructed into
/// physical units, and only then may the range driver update continuation.
pub(crate) fn recover_resolved_ph_after_numerical_failure<'a>(
    request: ResolvedPhaseEnthalpyRequest<'a>,
    physical_error: ReactionExtentError,
) -> Result<FixedPressureEnthalpySolution, ReactionExtentError> {
    if request.solve_options.extensive_normalization_policy()
        != crate::Thermodynamics::ChemEquilibrium::phase_equilibrium_workflow::
    ...
    let normalized_constraint = EquilibriumConstraint::ph_joules(
        request.constraint.pressure(),
        request.constraint.reference_pressure(),
        normalized_target,
        seed,
    )?;
    let mut normalized_temperature_options = request.temperature_options.clone();
    ...
    let normalized_options = request
        .solve_options
        .clone()
        .with_extensive_normalization_policy(
            crate::Thermodynamics::ChemEquilibrium::phase_equilibrium_workflow::
    ...
    let normalized_request = ResolvedPhaseEnthalpyRequest {
        ...
        enthalpy: request.enthalpy.clone(),
        solve_options: normalized_options,
        solve_mode: request.solve_mode.clone(),
    ...
    let normalized_solution = solve_resolved_ph(normalized_request).map_err(|recovery| {
        ReactionExtentError::ExtensiveNormalizationRecoveryFailed {
            stage: "P,H range normalized basin discovery",
            physical: Box::new(physical_error),
            recovery: Box::new(recovery),
        }
    })?;
    let discovery_backend = normalized_solution
        .equilibrium()
        .solve_report()
        .accepted_backend
        .clone();
    let accepted_conditions = normalized_solution.equilibrium().conditions();
    let recovered_moles = normalization.denormalize_moles(
        normalized_solution
            .equilibrium()
            .accepted_solution()
            .moles(),
    )?;
    let recovered_composition = MultiphaseInitialComposition::from_dense(&layout, recovered_moles)?;
    let physical_retry_constraint = EquilibriumConstraint::ph_joules(
        request.constraint.pressure(),
        request.constraint.reference_pressure(),
        crate::Thermodynamics::ChemEquilibrium::equilibrium_constraints::TotalEnthalpyJoules::new(
            target,
        )?,
        accepted_conditions.temperature(),
    )?;
    let mut physical_retry = request.clone();
    physical_retry.initial_composition = recovered_composition;
    physical_retry.constraint = physical_retry_constraint;
    ...
    let physical_retry_failure = match solve_resolved_ph(physical_retry) {
        Ok(mut physical_solution) => {
            let physical_retry_backend = physical_solution
                .equilibrium()
                .solve_report()
                .accepted_backend
                .clone();
            physical_solution.solution = physical_solution
                .solution
                .with_extensive_normalization_recovery(ExtensiveNormalizationRecoveryEvidence {
    ...
    .report()
    .clone();
    let mut recovered = normalized_solution
        .reconstruct_physical_extensive_boundary(normalization, physical_build_report)?;
    recovered.solution = recovered.solution.with_extensive_normalization_recovery(
...
fn solve_resolved_ph_monolithic_phase_control(
    ...
    let monolithic_options = temperature_options.monolithic_options()?;
    let temperature_bounds = request.temperature_bounds;
    let mut temperature_seed = initial_temperature;
    let mut temperature_seed_report = None;
    ...
    let solution = template.solve_with_fixed_active_solver(
        ...
        |runner, active, seed, species_phase, full_element_totals| {
            let attempt = solve_monolithic_active_set_candidate_with_seed_report(
                runner,
                active,
                seed,
                species_phase,
            ...
            let (candidate, report) = match attempt {
                Ok(outcome) => outcome,
                Err(error) => {
                    inner_backend_attempts =
                        inner_backend_attempts.saturating_add(error.started_backend_attempts());
                    inner_nonlinear_iterations =
                        inner_nonlinear_iterations.saturating_add(error.nonlinear_iterations());
                    enforce_monolithic_work_limits(
                        &temperature_options,
                        started,
                        inner_backend_attempts,
            ...
            inner_backend_attempts =
                inner_backend_attempts.saturating_add(report.started_backend_attempts());
            inner_nonlinear_iterations =
                inner_nonlinear_iterations.saturating_add(report.nonlinear_iterations());
            enforce_monolithic_work_limits(
                &temperature_options,
                started,
                inner_backend_attempts,
            ...
            temperature_seed_report = Some(report);
            Ok(candidate)
        },
    )?;
    ...
    enforce_monolithic_work_limits(
        ...
        inner_backend_attempts,
        inner_nonlinear_iterations,
        phase_control_report.transitions.len(),
    )?;
...
fn solve_resolved_ph_monolithic(
    ...
    let outcome = match runner.solve_from_temperature_seed(initial_temperature) {
        Ok(outcome) => outcome,
        Err(error) => {
            enforce_monolithic_work_limits(
                &options,
...
mod tests {
    ...
    fn monolithic_mode_requires_resolved_thermochemistry_capabilities() {
        assert_eq!(PhSolveMode::default(), PhSolveMode::Auto);
        assert_eq!(
            PhTemperatureSolveOptions::default()
                .with_monolithic_seed_policy(PhMonolithicSeedPolicy::InitialOnly)
                .monolithic_seed_policy,
    ...
    #[test]
    fn monolithic_work_gate_enforces_budgets_and_cancellation_before_publication() {
        let backend_limited = PhTemperatureSolveOptions::default()
            .with_max_inner_backend_attempts(1)
            .unwrap();
        assert!(matches!(
            enforce_monolithic_work_limits(&backend_limited, Instant::now(), 2, 0, 0),
            Err(ReactionExtentError::InvalidProblem {
                field: "inner_backend_budget",
                ..
            })
        ...
        let iteration_limited = PhTemperatureSolveOptions::default()
            .with_max_inner_nonlinear_iterations(2)
            .unwrap();
        assert!(matches!(
            enforce_monolithic_work_limits(&iteration_limited, Instant::now(), 1, 3, 0),
            Err(ReactionExtentError::InvalidProblem {
                field: "inner_nonlinear_iteration_budget",
                ..
            })
        ...
        let transition_limited = PhTemperatureSolveOptions::default()
            .with_max_phase_control_transitions(1)
            .unwrap();
        assert!(matches!(
            enforce_monolithic_work_limits(&transition_limited, Instant::now(), 1, 1, 2),
            Err(ReactionExtentError::InvalidProblem {
                field: "phase_control_transition_budget",
                ..
            })
        ...
        let cancelled = PhTemperatureSolveOptions::default().with_execution_control(control);
        assert!(matches!(
            enforce_monolithic_work_limits(&cancelled, Instant::now(), 1, 1, 0),
