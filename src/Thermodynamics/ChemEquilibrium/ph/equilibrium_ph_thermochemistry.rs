...
/// Declared pressure convention of one source record's standard state.
///
/// `G0(T)` alone is not enough to reconstruct this convention. In particular,
/// a NASA-like coefficient vector does not prove whether its source used
/// 1 bar or 1 atm. The equilibrium conditions select the activity reference
/// pressure; this value records the pressure at which the source's standard
/// thermochemistry was defined, so boundary validation can reject an
/// undocumented conversion instead of applying one silently.
#[derive(Debug, Clone, PartialEq)]
pub enum ThermochemistryStandardStatePressure {
    /// The selected JSON record explicitly declares its standard-state
    /// pressure in Pa. `declaration` names the field that supplied it.
    Declared {
        pressure_pa: f64,
        declaration: String,
    },
    /// The record has no machine-readable pressure declaration. This is an
    /// explicit provenance state, not a synonym for any default pressure.
    Undeclared,
}

impl ThermochemistryStandardStatePressure {
    const RECORD_PRESSURE_FIELDS: [&'static str; 2] =
        ["standard_state_pressure_pa", "reference_pressure_pa"];

    /// Creates a validated explicit pressure declaration for adapters and
    /// synthetic fixtures.
    pub fn declared(
        pressure_pa: f64,
        ...
        if !pressure_pa.is_finite() || pressure_pa <= 0.0 {
            return Err(ReactionExtentError::InvalidProblem {
                field: "thermochemistry_standard_state_pressure",
                message: format!(
                    "standard-state pressure must be finite and positive, got {pressure_pa}"
                ),
            });
        }
        ...
        if declaration.trim().is_empty() {
            return Err(ReactionExtentError::InvalidProblem {
                field: "thermochemistry_standard_state_pressure",
                message: "standard-state pressure declaration must not be empty".to_string(),
            });
        }
        Ok(Self::Declared {
            pressure_pa,
    ...
    /// Returns the declared pressure in Pa, if the source record supplied it.
    pub fn pressure_pa(&self) -> Option<f64> {
        match self {
            Self::Declared { pressure_pa, .. } => Some(*pressure_pa),
            Self::Undeclared => None,
        }
    }

    /// Returns whether a later activity or boundary calculation may rely on a
    /// machine-readable source pressure.
    pub fn is_declared(&self) -> bool {
        matches!(self, Self::Declared { .. })
    }

    /// Returns the exact metadata field that declared the pressure.
    ...
    fn from_record_payload(record: &Value) -> Result<Self, ReactionExtentError> {
        let mut declaration: Option<Self> = None;
        for field in Self::RECORD_PRESSURE_FIELDS {
            ...
            let pressure_pa =
                value
                    ...
                    .ok_or_else(|| ReactionExtentError::InvalidProblem {
                        field: "thermochemistry_standard_state_pressure",
                        message: format!(
                            "record field '{field}' must be a numeric pressure in Pa, got {value}"
                        ),
                    })?;
            let current = Self::declared(pressure_pa, format!("record.{field}"))?;
            if let Some(previous) = &declaration {
                if previous.pressure_pa() != current.pressure_pa() {
                    return Err(ReactionExtentError::InvalidProblem {
                        field: "thermochemistry_standard_state_pressure",
                        message: format!(
                            "record declares conflicting standard-state pressures through {:?} and {:?}",
        ...
        Ok(declaration.unwrap_or(Self::Undeclared))
    }
}

impl fmt::Display for ThermochemistryStandardStatePressure {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Declared {
                pressure_pa,
                declaration,
            } => write!(formatter, "{pressure_pa:.6} Pa ({declaration})"),
            Self::Undeclared => formatter.write_str("undeclared"),
        }
    }
}
...
pub struct ThermochemistryProvenance {
    ...
    record_key: String,
    state: String,
    standard_state_pressure: ThermochemistryStandardStatePressure,
}
...
impl ThermochemistryProvenance {
    ...
    pub fn new(
        ...
        Self::new_with_standard_state_pressure(
            component,
            library,
            record_key,
            state,
            ThermochemistryStandardStatePressure::Undeclared,
        )
    }

    /// Creates provenance with an explicit source standard-state convention.
    ///
    /// Production resolution uses this constructor after reading record
    /// metadata. The shorter [`Self::new`] constructor deliberately retains an
    /// `Undeclared` pressure for synthetic or compatibility capabilities.
    pub fn new_with_standard_state_pressure(
        component: PhaseComponentId,
        library: impl Into<String>,
        record_key: impl Into<String>,
        state: impl Into<String>,
    ...
    /// Machine-readable standard-state pressure convention of the selected
    /// record. `Undeclared` must remain visible to later boundary validation.
    pub fn standard_state_pressure(&self) -> &ThermochemistryStandardStatePressure {
        &self.standard_state_pressure
    }
}
    ...
    pub fn from_resolved_system(
        ...
        for (phase, original) in resolved.phase_data() {
            ...
            for substance in substances {
                ...
                let standard_state_pressure =
                    ThermochemistryStandardStatePressure::from_record_payload(record.data())
                        .map_err(|error| ReactionExtentError::InvalidProblem {
                            field: "thermochemistry_standard_state_pressure",
                            message: format!(
                                "invalid standard-state pressure metadata for '{}' in '{}': {error}",
                                substance,
                                record.library(),
                            ),
                        })?;
                provenance.insert(
                    substance.clone(),
                    ThermochemistryProvenance::new_with_standard_state_pressure(
...
mod tests {
    ...
    use super::ThermochemistryStandardStatePressure;

    #[test]
    fn standard_state_pressure_requires_explicit_numeric_record_metadata() {
        let declared = ThermochemistryStandardStatePressure::from_record_payload(&json!({
            "standard_state_pressure_pa": 100_000.0,
        }))
        .unwrap();
        assert_eq!(declared.pressure_pa(), Some(100_000.0));
        assert_eq!(
            declared.declaration(),
            Some("record.standard_state_pressure_pa")
        );

        let undeclared = ThermochemistryStandardStatePressure::from_record_payload(&json!({
            "Cp": [200.0, 1_000.0],
            "model": "NASA7",
        }))
        .unwrap();
        assert!(!undeclared.is_declared());
        assert_eq!(undeclared.pressure_pa(), None);
    }

    #[test]
    fn standard_state_pressure_rejects_invalid_or_conflicting_metadata() {
        let invalid = ThermochemistryStandardStatePressure::from_record_payload(&json!({
            "reference_pressure_pa": "one bar",
        }));
        assert!(invalid.is_err());

        let conflicting = ThermochemistryStandardStatePressure::from_record_payload(&json!({
            "standard_state_pressure_pa": 100_000.0,
