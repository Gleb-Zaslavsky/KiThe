...
#[cfg(test)]
use crate::Thermodynamics::ChemEquilibrium::equilibrium_ph_options::{
    PhAcceptanceOptions, PhMonolithicSeedPolicy,
};
use crate::Thermodynamics::ChemEquilibrium::equilibrium_ph_options::{
    PhMonolithicOptions, PhMonolithicSeedAttemptReport, PhMonolithicSeedRecoveryReport,
};
...
pub(crate) struct MonolithicPhSolveOutcome {
    ...
    /// Ordered physical-temperature starts attempted before acceptance.
    pub(crate) temperature_seed_report: PhMonolithicSeedRecoveryReport,
}
...
impl PreparedMonolithicPhRunner {
    ...
    pub(crate) fn solve_from_temperature_seed(
        ...
        let temperatures = self
            .formulation
            .temperature_seed_candidates(temperature_seed, self.options.seed_policy())?;
        self.solve_from_temperature_candidates(temperatures, None, |temperature| {
            self.formulation.initial_unknowns(temperature)
        })
    }
    ...
    pub(crate) fn solve_from_log_moles_and_temperature_seed(
        ...
        let temperatures = self
            .formulation
            .temperature_seed_candidates(temperature_seed, self.options.seed_policy())?;
        self.solve_from_temperature_candidates(temperatures, rst_problem, |temperature| {
            self.formulation
                .initial_unknowns_from_log_moles(log_moles, temperature)
        })
    }

    /// Executes fresh backend cascades from ordered temperature starts and
    /// stops at the first candidate accepted by the common physical gate.
    fn solve_from_temperature_candidates<F>(
        &self,
        temperatures: Vec<f64>,
        rst_problem: Option<
            &crate::Thermodynamics::ChemEquilibrium::equilibrium_rst_backend::RstPreparedProblem,
        >,
        mut build_unknowns: F,
    ) -> Result<MonolithicPhSolveOutcome, ReactionExtentError>
    where
        F: FnMut(f64) -> Result<Vec<f64>, ReactionExtentError>,
    {
        let policy = self.options.seed_policy();
        let total = temperatures.len();
        let mut attempts = Vec::with_capacity(total);
        let mut failures = Vec::with_capacity(total);
        for (start_index, temperature_seed_k) in temperatures.into_iter().enumerate() {
            let initial_unknowns = build_unknowns(temperature_seed_k)?;
            match self.solve_from_initial_unknowns(initial_unknowns, rst_problem) {
                Ok(mut outcome) => {
                    attempts.push(PhMonolithicSeedAttemptReport {
                        temperature_seed_k,
                        accepted: true,
                        failure_kind: None,
                        error: None,
                    ...
                    outcome.temperature_seed_report = PhMonolithicSeedRecoveryReport {
                        policy,
                        attempts,
                        selected_attempt: start_index,
                    };
                    return Ok(outcome);
                }
                Err(error) => {
                    let retryable = error.is_retryable_formulation_failure();
                    attempts.push(PhMonolithicSeedAttemptReport {
                        temperature_seed_k,
                        accepted: false,
                        failure_kind: Some(error.kind()),
                        error: Some(error.to_string()),
                        started_backend_attempts: error.started_backend_attempts(),
                        nonlinear_iterations: error.nonlinear_iterations(),
                    });
                    if !retryable {
                        return Err(error);
                    }
                    failures.push(PhMonolithicSeedFailure::new(temperature_seed_k, error));
                    if start_index + 1 == total {
                        if failures.len() == 1 {
                            return Err(failures.pop().expect("one seed failure").into_cause());
                        }
                        return Err(ReactionExtentError::PhMonolithicSeedRecoveryFailed {
                            attempts: failures,
                        });
                    }
                }
        ...
        unreachable!("validated monolithic seed policy always produces one candidate")
    }
    ...
    fn solve_from_initial_unknowns(
        ...
        Ok(MonolithicPhSolveOutcome {
            ...
            pt_validation,
            solve_report,
            temperature_seed_report: PhMonolithicSeedRecoveryReport {
                policy: self.options.seed_policy(),
                attempts: Vec::new(),
                selected_attempt: 0,
            },
        })
...
pub(crate) fn solve_monolithic_active_set_candidate(
    ...
    monolithic_options: &PhMonolithicOptions,
) -> Result<PreparedActiveSetCandidate, ReactionExtentError> {
    solve_monolithic_active_set_candidate_with_seed_report(
        runner,
        active,
        full_seed,
        species_phase,
...
/// Variant used by the production P,H facade to retain seed-recovery
/// evidence without coupling the generic phase-control runner to P,H types.
pub(crate) fn solve_monolithic_active_set_candidate_with_seed_report(
    runner: &mut PreparedPhaseControlRunner,
    active: &[bool],
    full_seed: &[f64],
    species_phase: &[usize],
    ...
    temperature_seed: &mut f64,
    monolithic_options: &PhMonolithicOptions,
) -> Result<(PreparedActiveSetCandidate, PhMonolithicSeedRecoveryReport), ReactionExtentError> {
        ...
        match solve_monolithic_active_set_candidate_inner(
            ...
            Err(primary_error) => {
                ...
                for fraction in [1.0e-8, 1.0e-4, 1.0e-2, 1.0e-1] {
                    ...
                    if let Ok((mut candidate, seed_report)) =
                        ...
                        )
                    {
                        ...
                        return Ok((candidate, seed_report));
                    }
                }
    ...
    let primary = solve_monolithic_active_set_candidate_inner(
        runner,
        active,
        full_seed,
        species_phase,
    ...
    if primary.is_ok() || active.iter().any(|is_active| !is_active) {
        return primary;
    }

    // A phase accepted by the TPD lifecycle starts from a deliberately small
    // positive inventory. When that all-active monolithic solve is numerically
    // rejected, retry a short deterministic set of larger *numerical* seeds.
    // These are not new phase decisions and cannot publish independently: the
    // caller still validates the returned candidate and owns every transition.
    let primary_error = match primary {
        Err(error) => error,
        Ok(_) => unreachable!("successful candidate returned before trace-seed recovery"),
    };
    let system_total = full_seed.iter().map(|value| value.exp()).sum::<f64>();
    let trace_threshold = (system_total * 1.0e-7).max(1.0e-12);
    let trace_active_phases = (0..active.len())
        .filter(|&phase| {
            active[phase]
                && full_seed
                    .iter()
                    .zip(species_phase)
                    .filter(|&(_, &candidate_phase)| candidate_phase == phase)
                    .map(|(log_moles, _)| log_moles.exp())
                    .sum::<f64>()
                    <= trace_threshold
        })
        .collect::<Vec<_>>();
    if trace_active_phases.is_empty() {
        return Err(primary_error);
    }
    for fraction in [1.0e-6, 1.0e-4, 1.0e-2, 1.0e-1] {
        let mut retry_seed = full_seed.to_vec();
        for phase_index in &trace_active_phases {
            let component_count = species_phase
                .iter()
                .filter(|&&phase| phase == *phase_index)
                .count();
            if component_count == 0 {
                return Err(ReactionExtentError::InvalidProblem {
                    field: "monolithic_ph_trace_retry",
                    message: format!("active phase {phase_index} has no declared components"),
                });
            }
            seed_activated_phase_with_composition(
                &mut retry_seed,
                PhaseIndex::new(*phase_index, active.len())?,
                species_phase,
                &vec![1.0 / component_count as f64; component_count],
                PhaseTotalSeedPolicy::RelativeToSystemTotal {
                    fraction,
        ...
        if let Ok(candidate) = solve_monolithic_active_set_candidate_inner(
            runner,
            active,
            &retry_seed,
            species_phase,
...
fn solve_monolithic_active_set_candidate_inner(
    ...
    let monolithic = PreparedMonolithicPhRunner::new(formulation, settings, *monolithic_options)?;
    let outcome = monolithic.solve_from_temperature_seed(*temperature_seed)?;
    let temperature_seed_report = outcome.temperature_seed_report.clone();
...
mod tests {
    ...
    fn one_species_formulation(with_symbolic_capability: bool) -> PreparedPhFormulation {
        one_species_formulation_at_target(with_symbolic_capability, 1_000.0)
    }

    fn one_species_formulation_at_target(
        with_symbolic_capability: bool,
        target_temperature: f64,
    ) -> PreparedPhFormulation {
        ...
        PreparedPhFormulation::new(
            ...
            10.0 * target_temperature,
            EnthalpyScale::new(10.0 * target_temperature).unwrap(),
        )
        .unwrap()
    }

    fn monolithic_options(
        scaled_tolerance: f64,
        absolute_tolerance: f64,
        seed_policy: PhMonolithicSeedPolicy,
    ) -> PhMonolithicOptions {
        PhMonolithicOptions::new(
            PhAcceptanceOptions::new(scaled_tolerance, absolute_tolerance).unwrap(),
            seed_policy,
        )
    }
    ...
    fn monolithic_runner_solves_composition_and_temperature_together() {
        ...
        assert_eq!(outcome.temperature_seed_report.attempts().len(), 1);
        assert!(!outcome.temperature_seed_report.recovered_after_retry());
    }

    #[test]
    fn bounded_temperature_recovery_uses_a_fresh_midpoint_transaction() {
        let mut settings = EquilibriumSolverSettings::default();
        settings.solver = Solvers::NR;
        settings.solver_params.max_iter = 1;

        let strict = PreparedMonolithicPhRunner::new(
            one_species_formulation(false),
            settings.clone(),
            monolithic_options(1.0e-8, 1.0e-6, PhMonolithicSeedPolicy::InitialOnly),
        )
        .unwrap();
        assert!(strict.solve_from_temperature_seed(600.0).is_err());

        let recovered = PreparedMonolithicPhRunner::new(
            one_species_formulation(false),
            settings,
            monolithic_options(
                1.0e-8,
        ...
        .unwrap()
        .solve_from_temperature_seed(600.0)
        .unwrap();

        assert!((recovered.snapshot.temperature - 1_000.0).abs() < 1.0e-10);
        assert_eq!(recovered.temperature_seed_report.attempts().len(), 2);
        assert!(!recovered.temperature_seed_report.attempts()[0].accepted());
        assert!(recovered.temperature_seed_report.attempts()[1].accepted());
        assert_eq!(
            recovered.temperature_seed_report.attempts()[1].temperature_seed_k(),
            1_000.0
        );
        assert!(recovered.temperature_seed_report.recovered_after_retry());
    }

    #[test]
    fn exhausted_temperature_recovery_preserves_every_typed_seed_failure() {
        let mut settings = EquilibriumSolverSettings::default();
        settings.solver = Solvers::NR;
        settings.solver_params.max_iter = 1;

        let error = PreparedMonolithicPhRunner::new(
            one_species_formulation_at_target(false, 1_137.0),
            settings,
            monolithic_options(
                1.0e-12,
                1.0e-12,
        ...
        let ReactionExtentError::PhMonolithicSeedRecoveryFailed { attempts } = error else {
            panic!("expected typed monolithic seed-recovery failure, got {error}");
        };
        assert_eq!(attempts.len(), 4);
        assert_eq!(attempts[0].temperature_seed_k(), 600.0);
        assert_eq!(attempts[1].temperature_seed_k(), 1_000.0);
        assert_eq!(attempts[2].temperature_seed_k(), 750.0);
        assert_eq!(attempts[3].temperature_seed_k(), 1_250.0);
        assert!(
            attempts
                .iter()
                .all(|attempt| { attempt.cause().is_retryable_formulation_failure() })
        );
    }
