...
impl EquilibriumDisplayPolicy {
    ...
    /// Formats a dimensionless quantity, downgrading the SI engineering style
    /// to plain fixed notation.
    ///
    /// Engineering-style prefixes are meaningless for dimensionless values, so
    /// this helper maps that style to the same decimal count in fixed notation
    /// while leaving other number styles unchanged.
...
pub struct EquilibriumFormattedPhaseRow {
    ...
    pub phase: String,
    /// Rendered lifecycle status string.
    pub status: String,
    /// Formatted physical total moles (zero for absent phases).
    pub physical_total_moles: String,
    /// Formatted numerical positive-mole coordinate retained for diagnostics.
...
pub struct EquilibriumFormattedComponentRow {
    /// Collision-free `phase::substance` display label.
    pub component: String,
    /// Semantic phase name this component belongs to.
    pub phase: String,
    /// Bare substance name used for thermochemical record lookup.
    pub substance: String,
    /// Formatted physical mole amount in the accepted state.
    pub physical_moles: String,
    /// Formatted numerical positive-mole coordinate retained internally.
    pub numerical_moles: String,
    /// Formatted local mole fraction within the component's phase.
    pub mole_fraction: String,
    /// Formatted physical initial mole amount before trace seeding.
    pub initial_moles: String,
    /// Formatted standard-state Gibbs energy in J/mol.
