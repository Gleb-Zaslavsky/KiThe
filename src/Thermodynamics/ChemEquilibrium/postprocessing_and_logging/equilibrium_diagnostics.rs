...
pub struct PhaseStabilityDiagnostic {
    /// Dense phase index in canonical phase order.
    pub phase_index: usize,
    /// Whether the phase is active in the accepted solution.
    pub active: bool,
    /// Total mole amount of the phase at the accepted state.
    pub phase_total_moles: f64,
    /// Minimum tangent-plane-distance per mol, when evaluated.
    pub minimum_tpd_j_per_mol: Option<f64>,
    /// Incipient composition (mole fractions) of the TPD minimizer, when run.
...
pub enum EquilibriumDiagnosticEvent {
    /// A solve transaction began with validated physical conditions.
    SolveStarted {
        /// Thermodynamic conditions of the accepted problem.
        conditions: EquilibriumConditions,
        /// Initial active-phase policy/status set.
        initial_phase_set: PhaseSet,
    },
    /// A bounded outer-loop iteration started for one active phase set.
    OuterIterationStarted {
        /// Zero-based outer-loop iteration number.
        iteration: usize,
        /// Active phase set being solved in this iteration.
        active_phase_set: PhaseSet,
    },
    /// A fixed active-set candidate passed the acceptance gate.
    ActiveSetCandidateAccepted {
        /// Zero-based outer-loop iteration number.
        iteration: usize,
        /// Active phase set whose candidate was accepted.
        active_phase_set: PhaseSet,
        /// Backend-independent acceptance evidence.
        validation: EquilibriumCandidateReport,
        /// Concise human-readable summary of the backend cascade.
    ...
    /// Stability (tangent-plane) evidence was evaluated for all phases.
    StabilityEvaluated {
        /// Zero-based outer-loop iteration number.
        iteration: usize,
        /// Hysteresis creation threshold in J/mol.
        dg_create_j_per_mol: f64,
        /// Hysteresis keep threshold in J/mol.
        dg_keep_j_per_mol: f64,
        /// Per-phase stability diagnostics in canonical phase order.
        phases: Vec<PhaseStabilityDiagnostic>,
    },
    /// The outer loop committed a phase transition.
    TransitionAccepted {
        /// Zero-based outer-loop iteration number.
        iteration: usize,
        /// Dense indices of phases activated by this transition.
        activated_phase_indices: Vec<usize>,
        /// Dense indices of phases deactivated by this transition.
        deactivated_phase_indices: Vec<usize>,
        /// Typed reason for accepting the transition.
        reason: PhaseTransitionReason,
        /// Phase set before the transition.
        previous_phase_set: PhaseSet,
        /// Phase set after the transition.
        new_phase_set: PhaseSet,
        /// Optional TPD-derived composition used to seed a new phase.
        incipient_composition: Option<Vec<f64>>,
    },
    /// A proposed activation was rejected by the hysteresis dead zone.
    TransitionHeldByHysteresis {
        /// Zero-based outer-loop iteration number.
        iteration: usize,
        /// Dense index of the phase whose activation was held.
        phase_index: usize,
    },
    /// A boundary-recovery probe with a smaller active set was accepted.
    RecoveryProbeAccepted {
        /// Zero-based outer-loop iteration number.
        iteration: usize,
        /// Dense index of the phase removed by the recovery probe.
        phase_index: usize,
        /// Phase set before the recovery probe.
        previous_phase_set: PhaseSet,
        /// Reduced phase set established by the accepted probe.
        new_phase_set: PhaseSet,
    },
    ...
    /// Exact extensive normalization produced the accepted public physical
    /// result after the original physical-coordinate solve failed.
    ExtensiveNormalizationRecoveryAccepted {
        /// Sum of positive physical input component moles used as the scale.
        physical_inventory_scale: f64,
        /// Failure family that authorized recovery.
        trigger_kind: ReactionExtentErrorKind,
        /// Backend that accepted the normalized equivalent problem.
        discovery_backend: String,
        /// Backend that accepted the optional physical retry.
        physical_retry_backend: Option<String>,
        /// Whether public fields were reconstructed through the audited
        /// physical publication boundary.
        reconstructed_physical_boundary: bool,
    },
    ...
    /// The transaction ended by publishing an accepted solution.
    SolveAccepted {
        /// Final settled active phase set.
        final_phase_set: PhaseSet,
        /// Total outer-loop iterations used by the solve.
        outer_iterations: usize,
        /// Total accepted phase transitions during the solve.
...
impl EquilibriumDiagnosticsCollector {
    ...
    /// Continues one retained report with a workflow-level decision known only
    /// after the inner phase-control transaction has completed.
...
/// Tests whether one diagnostics mode is at least as detailed as another.
///
/// The modes form a strict ladder `Disabled < Summary < PhaseLifecycle <
/// Detailed`. A mode "includes" a required level when its rank is greater than
/// or equal to the required rank, so `PhaseLifecycle` captures summary events
/// and `Detailed` captures everything.
...
mod tests {
    ...
    #[test]
    fn workflow_level_recovery_event_extends_an_existing_report() {
        let options = EquilibriumDiagnosticsOptions::enabled(EquilibriumDiagnosticsMode::Summary)
            .with_max_events(2);
        let phase_set =
            PhaseSet::from_policy(&InitialPhaseSet::AllCandidatePhases, &[true]).unwrap();
        let mut initial = EquilibriumDiagnosticsCollector::new(options.clone());
        initial.record(
            EquilibriumDiagnosticsMode::Summary,
            EquilibriumDiagnosticEvent::SolveAccepted {
        ...
        let mut extended = EquilibriumDiagnosticsCollector::from_report(options, &initial.finish());
        extended.record(
            EquilibriumDiagnosticsMode::Summary,
            EquilibriumDiagnosticEvent::ExtensiveNormalizationRecoveryAccepted {
                ...
                discovery_backend: "Legacy(LM)".to_string(),
                physical_retry_backend: None,
                reconstructed_physical_boundary: true,
            },
        );
        let report = extended.finish();
