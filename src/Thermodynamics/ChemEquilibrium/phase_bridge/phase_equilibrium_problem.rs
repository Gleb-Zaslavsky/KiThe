...
use crate::Thermodynamics::ChemEquilibrium::equilibrium_workflows::{PhaseManager, PhaseSet};
...
pub struct PhaseEquilibriumBuildReport {
    ...
    /// this full list and its totals for conservation evidence and diagnostics.
    element_labels: Vec<String>,
    /// Conserved physical element totals before numerical trace seeding.
    element_totals: Vec<f64>,
    /// Deterministic independent subset of `element_labels` used by the
    /// square log-moles solver formulation.
...
impl PhaseEquilibriumBuildReport {
    ...
    /// Independent elemental constraints retained by the numerical solver.
    ///
    /// Omitted full columns are linearly dependent on earlier retained columns;
    /// no physical conservation direction is discarded.
    pub fn solver_element_labels(&self) -> &[String] {
        &self.solver_element_labels
    }
...
impl PhaseEquilibriumProblemBundle {
    ...
    where
        ...
        G: FnOnce(&mut PhaseManager),
    {
        self.solve_with_bounded_phase_control_with_diagnostics_and_seed(
            configure_solver,
            configure_phase_control,
            diagnostics,
            None,
    ...
    /// Internal recovery variant that changes only the nonlinear basin seed.
    /// The physical inventory still determines the initial active phase set.
    pub(crate) fn solve_with_bounded_phase_control_with_diagnostics_and_seed<F, G>(
        self,
        configure_solver: F,
        configure_phase_control: G,
        diagnostics: EquilibriumDiagnosticsOptions,
        basin_seed: Option<LogMolesInitialGuess>,
        ...
        G: FnOnce(&mut PhaseManager),
    {
        ...
        configure_phase_control(runner.configure_phase_control());
        if let Some(seed) = basin_seed {
            runner.set_independent_basin_seed(seed)?;
        }
        if let Some(phase_set) = continuation_phase_set {
            runner.set_continuation_phase_set(phase_set)?;
        }
...
impl PreparedPhaseControlTemplate {
    ...
    pub(crate) fn solve_at(
        ...
        if let Some(phase_set) = continuation_phase_set {
            ...
            // The first point is an independent solve. `retarget_numeric`
            // receives its numerical coordinate seed for formulation refresh,
            // but it must not turn that construction seed into physical
            // continuation before any state has been accepted.
            self.runner.clear_continuation_state();
        }
...
pub(crate) fn build_phase_equilibrium_problem_with_timing(
    ...
    let solver_element_labels = independent_element_columns
        .iter()
        .map(|&column| element_labels[column].clone())
        .collect::<Vec<_>>();
    let solver_element_composition = DMatrix::from_fn(
        component_count,
...
/// bridge problems.
///
/// A system may declare both H and O while every present component is H2O.
/// Those columns represent the same closed-system direction and make the raw
/// log-moles equations overdetermined. The first columns that raise SVD rank
/// are retained for the solver; the full matrix and totals stay in the report.
fn deterministic_element_basis(
    ...
    if !tolerance.is_finite() || tolerance <= 0.0 {
        return Err(ReactionExtentError::InvalidProblem {
            field: "element_basis",
            message: "rank tolerance must be finite and positive".to_owned(),
        });
    }
    let full_rank = SVD::new(element_composition.clone(), false, false)
        .singular_values
        .iter()
    ...
    if full_rank == 0 {
        return Err(ReactionExtentError::InvalidProblem {
            field: "element_basis",
            message: "element composition has no independent conservation direction".to_owned(),
        });
    }

    let mut columns = Vec::with_capacity(full_rank);
    let mut current_rank = 0;
    for candidate_column in 0..element_composition.ncols() {
        let mut candidate = columns.clone();
        candidate.push(candidate_column);
        let candidate_matrix = DMatrix::from_fn(
            element_composition.nrows(),
            candidate.len(),
            |row, column| element_composition[(row, candidate[column])],
        );
    ...
    if current_rank != full_rank {
        return Err(ReactionExtentError::InvalidProblem {
            field: "element_basis",
            message: format!(
                "could not select {full_rank} independent element columns; found {current_rank}"
            ),
        });
    }
    Ok(columns)
