...
use crate::Thermodynamics::ChemEquilibrium::equilibrium_solver_policy::{
    EquilibriumSolveReport, MultiStartSolveReport, SolverBackend,
};
...
use crate::Thermodynamics::ChemEquilibrium::equilibrium_workflows::{
    MultiphaseAcceptanceReport, PhaseControlledSolveReport, PhaseStatus, PhaseTransitionReason,
};
...
/// Evidence that an extensive-conditioning recovery was used successfully.
///
/// Every amount published by [`MultiphaseEquilibriumSolution`] is in physical
/// units. The optional retry fields distinguish a directly accepted physical
/// retry from an accepted normalized solution reconstructed through the
/// audited physical publication boundary.
#[derive(Debug, Clone, PartialEq)]
pub struct ExtensiveNormalizationRecoveryEvidence {
    /// Sum of positive physical input component moles used as the exact scale.
    pub physical_inventory_scale: f64,
    /// Failure family that authorized normalization recovery.
    pub trigger_kind: ReactionExtentErrorKind,
    /// Human-readable original physical failure retained for diagnosis.
    pub trigger_message: String,
    /// Backend that accepted the normalized basin-discovery state.
    pub discovery_backend: SolverBackend,
    /// Backend that accepted the final physical-coordinate retry, when that
    /// proof succeeded without returning to the normalized representation.
    pub physical_retry_backend: Option<SolverBackend>,
    /// Why the optional physical retry could not be used for publication.
    pub physical_retry_failure: Option<String>,
    /// `true` when the accepted normalized state was reconstructed through the
    /// explicit physical publication boundary.
    pub reconstructed_physical_boundary: bool,
}
...
pub struct MultiphaseEquilibriumSolution {
    ...
    /// Optional proof that a normalized basin seed was needed before the final
    /// physical-coordinate solve could be accepted.
    extensive_normalization_recovery: Option<ExtensiveNormalizationRecoveryEvidence>,
}
...
impl MultiphaseEquilibriumSolution {
    ...
    fn from_parts(
        ...
        Ok(Self {
            ...
            timing,
            diagnostics,
            extensive_normalization_recovery: None,
        })
    }
    ...
    /// Returns extensive-conditioning recovery evidence, when recovery was
    /// needed. `None` means the original physical solve succeeded directly.
    pub fn extensive_normalization_recovery(
        &self,
    ) -> Option<&ExtensiveNormalizationRecoveryEvidence> {
        self.extensive_normalization_recovery.as_ref()
    }

    /// Attaches recovery provenance after the final physical-coordinate solve
    /// has passed the ordinary publication checks.
    pub(crate) fn with_extensive_normalization_recovery(
        mut self,
        evidence: ExtensiveNormalizationRecoveryEvidence,
    ) -> Self {
        self.extensive_normalization_recovery = Some(evidence);
        self
    }

    /// Reconstructs public extensive values after an accepted normalized solve.
    ///
    /// Intensive thermodynamics, topology, mole fractions, and backend metrics
    /// are invariant. Mole-valued fields and log-mole seeds are mapped back to
    /// physical units. Raw nonlinear norms remain solver-coordinate evidence;
    /// their representation is made explicit by the attached recovery record.
    pub(crate) fn reconstruct_physical_extensive_boundary(
        mut self,
        normalization: ExtensiveNormalization,
        physical_build_report: PhaseEquilibriumBuildReport,
    ) -> Result<Self, ReactionExtentError> {
        if physical_build_report.layout_fingerprint() != self.layout_fingerprint()
            || physical_build_report.conditions() != self.conditions()
        {
            return Err(ReactionExtentError::InvalidCandidate {
                field: "extensive_normalization_publication",
                message: "physical build evidence does not match the normalized solution"
                    .to_string(),
            });
        }

        let physical_log_moles = normalization
            .denormalize_log_moles(self.accepted_solution.log_moles())?
            .into_inner();
        let physical_moles = normalization.denormalize_moles(self.accepted_solution.moles())?;
        let physical_validation = denormalize_candidate_report(
            self.accepted_solution.validation(),
            normalization.physical_inventory_scale(),
        );
        self.accepted_solution = EquilibriumSolution::new(
            physical_log_moles,
            physical_moles,
            self.conditions(),
            physical_validation,
        ...
        self.physical_component_moles =
            normalization.denormalize_moles(&self.physical_component_moles)?;
        self.phase_totals = normalization.denormalize_phase_totals(&self.phase_totals)?;
        self.numerical_phase_totals =
            normalization.denormalize_phase_totals(&self.numerical_phase_totals)?;
        self.build_report = physical_build_report;

        if let Some(report) = &mut self.phase_control_report {
            denormalize_phase_control_report(report, normalization.physical_inventory_scale());
        }
        if let Some(acceptance) = &mut self.acceptance_report {
            if let Some(report) = &self.phase_control_report {
                acceptance.phase_control = report.clone();
                acceptance.final_validation = report.final_validation.clone();
            }
        }
        // Diagnostic events may contain extensive values. Until every event
        // variant owns a typed unit, dropping them is safer than publishing a
        // mixed-unit timeline. Recovery provenance remains available.
    ...
    pub fn summary_rows(&self) -> Vec<MultiphaseEquilibriumSummaryRow> {
        ...
        if let Some(evidence) = &self.extensive_normalization_recovery {
            rows.push(MultiphaseEquilibriumSummaryRow {
                section: "conditioning",
                label: "extensive_normalization_recovery".to_string(),
                value: format!(
                    "scale={:.6e}, trigger={:?}, discovery={:?}, physical_retry={:?}, reconstructed={}",
                    evidence.physical_inventory_scale,
                    evidence.trigger_kind,
                    evidence.discovery_backend,
                    evidence.physical_retry_backend,
...
fn denormalize_candidate_report(
    report: &crate::Thermodynamics::ChemEquilibrium::equilibrium_validation::EquilibriumCandidateReport,
    scale: f64,
) -> crate::Thermodynamics::ChemEquilibrium::equilibrium_validation::EquilibriumCandidateReport {
    let mut report = report.clone();
    report.max_abs_element_balance_error *= scale;
    report.min_moles *= scale;
    report
}

fn denormalize_phase_control_report(report: &mut PhaseControlledSolveReport, scale: f64) {
    for transition in &mut report.transitions {
        for total in &mut transition.phase_totals {
            *total *= scale;
        }
        let log_scale = scale.ln();
        for value in &mut transition.restart_seed {
            *value += log_scale;
        }
        transition.reason = match transition.reason {
            ...
            PhaseTransitionReason::BoundaryUnstableActivePhase {
                ...
                initial_phase_moles: initial_phase_moles * scale,
                minimum_tpd,
            },
        };
        transition.candidate_validation =
            denormalize_candidate_report(&transition.candidate_validation, scale);
    }
    report.final_validation = denormalize_candidate_report(&report.final_validation, scale);
}
