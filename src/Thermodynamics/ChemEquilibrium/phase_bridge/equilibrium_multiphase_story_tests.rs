...
fn water_phase_pair_switches_between_vapor_and_condensed_dominance_with_temperature() {
    ...
    let fixture = RealPurePhaseFamily::WaterLiquid
        .resolve_offline(repository, &["O2"])
        .expect("the shared water/liquid fixture must resolve without NIST");
    let inventory = RealPurePhaseInventory::new(vec![0.5], 0.0)
        .and_then(|inventory| inventory.with_inert("O2", 0.25, vec![0.0, 2.0]))
        .expect("the shared water/O2 inventory must validate");
    let composition = fixture
        .initial_composition(&inventory)
        .expect("the shared inventory must match the resolved P,T layout");
    let layout = MultiphaseEquilibriumLayout::new(fixture.resolved().phase_specs().to_vec())
        .expect("the shared P,T phase layout must validate");
