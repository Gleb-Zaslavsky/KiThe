...
impl ElementalReferenceGeometry {
    ...
    /// Fits element potentials `lambda` such that `A_ref * lambda ~= mu_ref`.
    ///
    /// Uses the cached SVD of the active reference composition matrix to solve
    /// the least-squares problem in the active subspace (skipping zero singular
    /// values). The residual against the supplied reference chemical potentials
    /// is checked against a scale-aware tolerance; an unrepresentable active
    /// state is rejected as an invalid candidate rather than silently returning
    /// a poor fit.
    ...
    /// Projects a candidate's elemental composition onto the reference
    /// nullspace to obtain the linear feasibility constraints.
    ///
    /// The candidate matrix must share the reference element columns. Its rows
    /// are multiplied by the cached nullspace basis, then reduced to an
    /// independent row basis whose rank reflects how many elemental directions
    /// of the candidate lie outside the active reference assemblage.
    ...
    /// Checks whether a candidate composition can be represented by the active
    /// reference elemental potentials.
    ///
    /// Validates the simplex constraint on `composition`, computes its implied
    /// element totals, and reconstructs the candidate's chemical potentials
    /// from the active element potentials. The returned
    /// [`ElementalDirectionFeasibility`] records the largest reconstruction
    /// residual against a scale-aware tolerance so the caller can decide
    /// whether the direction is physically representable.
...
impl PhaseStabilityGeometryCache {
    /// Returns the cached elemental reference geometry for a reference-species
    /// set, building it once and reusing it across repeated TPD evaluations.
    ///
    /// Validates that the reference species are in bounds and unique, extracts
    /// their rows from the state's element-composition matrix, and constructs
    /// the SVD geometry on the first use. Subsequent calls for the same key hit
    /// the cache and increment the reuse counter instead of refactoring the
    /// matrix.
...
impl IdealTpdProblem {
    ...
    /// Computes the reduced TPD potentials `g0 + R*T*ln(a_phase) - A_c^T*lambda`
    /// for every candidate component.
    ///
    /// Each reduced potential subtracts the elemental reference contribution
    /// built from the fitted element potentials, so the unconstrained TPD
    /// minimum is zero exactly when the candidate is representable by the
    /// active assemblage. Non-finite results are rejected as invalid candidates.
    ...
    /// Evaluates the interior (softmax) candidate composition given the reduced
    /// potentials, dual variables, and active-set linear constraints.
    ///
    /// Delegates to the free [`softmax_composition`] helper with the current
    /// temperature folded in. Used to generate incipient compositions during
    /// the constrained TPD minimization.
    ...
    /// Evaluates the TPD value and incipient properties for one composition.
    ///
    /// Validates the simplex constraint, then computes the objective
    /// `sum_i x_i * reduced_i` (excluding zero-mole support), the component
    /// reduced potentials, and packages them into an [`IdealTpdMinimum`] used
    /// by both the constrained minimizer and reporting.
...
impl DeterministicSimplex {
    /// Builds the two-phase simplex tableau for a linear feasibility program.
    ///
    /// The program has `constraints` inequalities `A*x + s <= b` with an
    /// artificial variable for phase one and a single objective row. Dimensions
    /// and finiteness are validated up front so the pivot loop never reads an
    /// out-of-bounds or non-finite tableau entry.
    fn new(
    ...
    /// Solves the linear feasibility program with the two-phase simplex method.
    ///
    /// Phase one drives the artificial variable to zero to establish feasibility;
    /// an unbounded or infeasible outcome is reported explicitly. If feasible,
    /// phase two minimizes the objective. Returns [`LinearProgramOutcome`] with
    /// the optimal value, or reports infeasibility/unboundedness.
    ...
    /// Runs one simplex phase (feasibility or optimality) to completion.
    ///
    /// Repeatedly selects the entering column by the most-negative reduced cost
    /// and the leaving row by the minimum-ratio test, pivoting until the
    /// objective row is non-negative or a direction is found to be unbounded.
    /// Returns `true` when the phase converged and `false` when no leaving row
    /// exists (unbounded).
    ...
    /// Performs one Gauss-Jordan pivot on the simplex tableau.
    ///
    /// Normalizes the pivot row and eliminates the pivot column from all other
    /// rows, then swaps the entering variable into the basis. A non-finite or
    /// near-zero pivot is rejected as an invalid problem because it would corrupt
    /// the tableau and produce a meaningless linear-programming result.
...
/// Computes an interior candidate composition via a log-space softmax.
///
/// For the constrained ideal TPD, the incipient composition is proportional to
/// `exp(-(reduced + B^T*dual)/(R*T))`. This helper evaluates those weights in
/// log space (subtracting the maximum for numerical stability), normalizes to a
/// simplex, and validates the result. A degenerate normalization or a
/// non-finite weight is rejected as an invalid candidate.
fn softmax_composition(
    reduced: &[f64],
    rt: f64,
    dual: &DVector<f64>,
...
/// Evaluates the stationarity KKT residual of the constrained ideal TPD.
///
/// Checks `d + R*T*(ln(x_i)+1) + B^T*dual` is constant across all supported
/// components (stationarity up to the mean). The returned maximum absolute
/// deviation from the mean measures how far the composition is from the
/// optimality conditions; a non-finite result is rejected as an invalid
/// candidate.
...
/// Validates that a candidate composition is a non-empty finite simplex.
///
/// Requires at least one component, all finite and non-negative fractions, and
/// (in the body) a total summing to one. Violations are reported as invalid
/// candidates so the TPD path never continues with a malformed composition.
...
pub struct ElementPotentialReport {
    /// Fitted element potentials in element-column order.
    pub potentials: Vec<f64>,
    /// Largest absolute residual of the active-potential reconstruction.
    pub max_abs_residual: f64,
    /// Rank of the active reference assemblage used for the fit.
    pub rank: usize,
}
...
pub struct PhaseStabilityLayout {
    /// Number of species in the full system.
    pub system_species_count: usize,
    /// Number of phases in the full system.
    pub system_phase_count: usize,
    /// Number of element columns in the composition matrix.
    pub element_count: usize,
    /// Component index range (as [start, end)) for every phase.
...
pub struct ElementalFeasibilityReport {
    /// Element totals implied by the candidate composition.
    pub candidate_element_totals: Vec<f64>,
    /// Largest absolute reconstruction residual against active potentials.
    pub max_abs_residual: f64,
    /// Scale-aware tolerance that `max_abs_residual` must satisfy.
...
pub struct TpdMinimizerReport {
    /// Number of independent linear constraints retained by the minimizer.
    pub independent_constraint_count: usize,
    /// Number of components with strictly positive support in the minimum.
    pub active_component_count: usize,
    /// Iterations used by the constrained minimization.
    pub iterations: usize,
    /// Largest absolute residual of the elemental constraints at the minimum.
    pub max_abs_constraint_residual: f64,
    /// Tolerance that the constraint residual must satisfy.
    pub constraint_tolerance: f64,
    /// Largest absolute KKT residual of the optimality conditions.
...
pub enum PhaseStabilityStatus {
    ...
    ExcludedByPolicy,
    /// The current domain already has an active ideal-gas assemblage. Active
    /// gas phases and separately declared inactive gas phases are therefore
    /// not independent TPD candidates. When no gas assemblage is active, an
    /// inactive gas phase is evaluated normally against the condensed
    /// reference assemblage so complete condensation remains representable.
    FixedGasAssemblage,
...
pub struct PhaseStabilityReport {
    /// Dense index of the phase this report describes.
    pub phase: PhaseIndex,
    /// Whether the phase is active in the current phase set.
    pub active: bool,
    /// High-level classification of the stability evaluation.
    pub status: PhaseStabilityStatus,
    /// Thermodynamic conditions under which stability was evaluated.
    pub conditions: PhaseStabilityConditions,
    /// Structural layout of the system at evaluation time.
    pub layout: PhaseStabilityLayout,
    /// Minimum tangent-plane distance per mol, when evaluated.
    ...
    /// Reconstructed element-potential evidence, when the phase was evaluated.
    pub element_potentials: Option<ElementPotentialReport>,
    /// Candidate elemental-direction feasibility, when evaluated.
    pub elemental_feasibility: Option<ElementalFeasibilityReport>,
    /// Constrained TPD minimizer details, when the phase was evaluated.
