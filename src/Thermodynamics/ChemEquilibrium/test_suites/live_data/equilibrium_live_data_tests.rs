...
///
/// The strict monolithic route must reject its non-physical all-active probe.
/// `Auto` may then change formulation, but it must retain that typed rejection
/// and publish only an independently accepted nested phase-control result.
#[test]
...
fn live_bounded_water_ph_auto_preserves_tpd_probe_rejection() {
    ...
    assert!(matches!(
        &monolithic,
        ReactionExtentError::PhMonolithicPhaseProbeRejected { .. }
    ));
    ...
    .expect("explicit nested phase control must recover the physical water state");
    ...
    .expect("Auto must recover through the independent nested formulation");
    assert_eq!(auto.report().solve_path(), PhSolvePath::NestedTemperature);
    let fallback = auto
        .report()
        .fallback_reason()
        .expect("Auto must retain the rejected monolithic probe as evidence");
    assert_eq!(
        fallback.error_kind(),
        ReactionExtentErrorKind::InvalidCandidate
    );
    assert!(
        fallback.message().contains("TPD creation threshold"),
        "Auto route lost the physical rejection boundary: {}",
        fallback.message()
    );
    assert!(matches!(
        auto.report().route_decisions(),
        [PhRouteDecision::AutoFallback { reason, .. }]
            if reason.error_kind() == ReactionExtentErrorKind::InvalidCandidate
                && reason.message() == monolithic.to_string()
    ));

    assert!((auto.temperature() - nested.temperature()).abs() <= 1.0e-5);
    for (component, (auto_moles, nested_moles)) in auto
        .equilibrium()
        .component_moles()
        .iter()
        ...
        .enumerate()
    {
        let tolerance = 1.0e-9 + 1.0e-7 * auto_moles.abs().max(nested_moles.abs());
        assert!(
            (auto_moles - nested_moles).abs() <= tolerance,
            "component {component} differs between Auto and explicit nested recovery: auto={auto_moles:e}, nested={nested_moles:e}, tolerance={tolerance:e}"
        );
    }
    let liquid = PhaseId::new(Some("liquid".to_string()));
    assert!(auto.equilibrium().phase_total(&liquid).unwrap_or_default() > 0.3);
    let phase_control = auto
        .equilibrium()
        .phase_control_report()
        .expect("accepted Auto recovery must retain phase-control evidence");
    assert!(
        phase_control
            .transitions
            .iter()
    ...
    assert!(
        auto.report().trials().iter().all(|trial| trial
            .inner_evidence()
            .and_then(|evidence| evidence.acceptance_report())
            .is_some()),
        "every published nested trial must carry accepted phase-control evidence"
    );
    assert_eq!(before, live_library_file_snapshot());
}
        ...
        match result {
            Ok(range) => {
                ...
                if matches!(backend, SolverBackend::RustedSciThe(_)) {
                    ...
                    let direct_reuse_count = range
                        .points()
                        .iter()
                        .filter(|point| point.report().symbolic_parameter_reused())
                        .count();
                    let recovery_count = range
                        .points()
                        .iter()
                        .filter(|point| {
                            matches!(
                    ...
                    assert_eq!(
                        ...
                        direct_reuse_count,
                        "symbolic update count must equal direct point-level reuse count"
                    );
                    assert_eq!(
                        range.report().formulation_builds(),
                        1 + recovery_count,
                        "each recovered RST point must account for one rebuilt formulation"
                    );
                    assert_eq!(
                        range.report().formulation_reuses(),
                        range.points().len().saturating_sub(1 + recovery_count),
                        "direct continuation reuse and recovery rebuilds must partition the range"
                    );
                    assert!(range.points().iter().all(|point| {
                        matches!(
                            point.report().preparation(),
...
fn live_reactive_gas_ph_target_range_backend_matrix() {
    ...
    for (name, backend) in backends {
        ...
        .expect("typed P,H range request must validate")
        .with_solve_options(options)
        // This is a strict backend characterization of the coupled system.
        // Keep it independent of the production `Auto` default, whose nested
        // fallback has a different reuse counter: one reuse per inner P,T
        // scalar trial rather than one per target-enthalpy point.
        .with_ph_solve_mode(PhSolveMode::Monolithic);
