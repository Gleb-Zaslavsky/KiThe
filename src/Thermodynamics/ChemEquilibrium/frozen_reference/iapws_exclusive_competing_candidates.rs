...
mod tests {
    ...
    use crate::Thermodynamics::ChemEquilibrium::equilibrium_log_moles::{GibbsFn, Phase, R, Solvers};
    ...
    fn liquid_boundary_pa(t: f64) -> f64 {
        ...
        const N: [f64; 6] = [-7.859_517_83, 1.844_082_59, -11.786_649_7, 22.680_741_1, -15.961_871_9, 1.801_225_02];
    ...
    fn runner_with_initial(
        ...
        let order = if liquid_before_ice { [0, 1, 2, 3] } else { [0, 1, 3, 2] };
        let names = order.iter().map(|&i| physical_names[i].to_owned()).collect();
        let initial = order.iter().map(|&i| physical_initial[i]).collect::<Vec<_>>();
        let seed = initial.iter().map(|n| n.max(TRACE_MOLES)).collect::<Vec<_>>();
        let elements = DMatrix::from_row_slice(
            4,
            2,
            &order.iter().flat_map(|&i| match i {
                0 => [1.0, 0.0],
                1 | 2 | 3 => [0.0, 1.0],
                _ => unreachable!(),
            }).collect::<Vec<_>>(),
        );
        let gibbs = order.iter().map(|&i| match i {
        ...
        let problem = EquilibriumProblem::new(
            ...
            gibbs,
            vec![
                Phase { kind: PhaseActivityModel::IdealGas, species: vec![0, 1] },
                Phase { kind: PhaseActivityModel::IdealSolution, species: vec![2] },
                Phase { kind: PhaseActivityModel::IdealSolution, species: vec![3] },
            ],
    ...
    fn physical_moles(outcome: &PreparedPhaseControlOutcome, liquid_before_ice: bool) -> [f64; 4] {
        ...
        if liquid_before_ice { [m[0], m[1], m[2], m[3]] } else { [m[0], m[1], m[3], m[2]] }
    }

    fn controlled_runner(delta_g: f64, a_before_b: bool) -> PreparedPhaseControlRunner {
        let base = R * T_K * (ice_boundary_pa(T_K) / P0_PA).ln();
        let order = if a_before_b { [0, 1, 2, 3] } else { [0, 1, 3, 2] };
        let names = order.iter().map(|&i| ["Ar(g)", "H2O(g)", "condensed_a", "condensed_b"][i].to_owned()).collect();
        let physical_initial = [AR_MOLES, WATER_MOLES, 0.0, 0.0];
        let initial = order.iter().map(|&i| physical_initial[i]).collect::<Vec<_>>();
        let seed = initial.iter().map(|n| n.max(TRACE_MOLES)).collect::<Vec<_>>();
        let elements = DMatrix::from_row_slice(4, 2, &order.iter().flat_map(|&i| match i {
            0 => [1.0, 0.0],
            _ => [0.0, 1.0],
        }).collect::<Vec<_>>());
        let gibbs = order.iter().map(|&i| match i {
    ...
    fn i5_iapws_water_has_two_unstable_condensed_candidates_and_order_invariant_fixed_point() {
        ...
        println!("water candidate preflight: T={T_K} p_liquid={p_liquid:.12} p_ice={p_ice:.12} delta_p={:.12}", p_liquid - p_ice);
        assert!(p_ice < p_liquid, "IAPWS must place ice below liquid below triple point");
        assert!(tpd_liquid < dg_create && tpd_ice < dg_create);
        assert!((tpd_liquid - tpd_ice).abs() > 1e-4, "candidate ordering is below diagnostic resolution");
        ...
        let initial_tpds_a = out_a
            ...
            .minimum_tpds
            .clone();
        assert!(initial_tpds_a.get(1).and_then(|v| *v).is_some_and(|v| v < dg_create));
        assert!(initial_tpds_a.get(2).and_then(|v| *v).is_some_and(|v| v < dg_create));
        assert_eq!(out_a.phase_control_report.final_phase_set.active_mask(), vec![true, false, true]);
        assert_eq!(out_b.phase_control_report.final_phase_set.active_mask(), vec![true, true, false]);
        for (x, y) in ma.into_iter().zip(mb) { assert!((x - y).abs() < 1e-8); }
        ...
        println!("Ar + H2O exclusive candidates: T={T_K:.2} K P={P_TOTAL_PA:.1} Pa p_liquid={p_liquid:.6} Pa p_ice={p_ice:.6} TPD_liquid={tpd_liquid:.6e} TPD_ice={tpd_ice:.6e} final=gas+ice order_invariant=true");
    }
    ...
    fn i5_iapws_water_metastable_replacement_and_stable_ice_retention_matrix() {
        ...
        let histories = [
            ...
            ("gas+liquid", [AR_MOLES, gas_liquid_water, WATER_MOLES - gas_liquid_water, 0.0]),
            ("gas+ice", [AR_MOLES, gas_ice_water, 0.0, WATER_MOLES - gas_ice_water]),
        ];
        ...
        for (history, initial) in histories {
            for liquid_before_ice in [true, false] {
                ...
                assert_eq!(outcome.phase_control_report.final_phase_set.active_mask(),
                    if liquid_before_ice { vec![true, false, true] } else { vec![true, true, false] });
                ...
                route_count += 1;
                println!("history={history:<9} order={} final=gas+ice transitions={} liquid_moles={:.6e} ice_moles={:.6e}",
                    if liquid_before_ice { "L->I" } else { "I->L" },
                    outcome.phase_control_report.transitions.len(), physical[2], physical[3]);
            }
        }
    ...
    fn i5_controlled_condensed_gibbs_split_matrix_has_analytic_losing_tpd() {
        ...
        for delta in deltas {
            ...
            let outcome_a = a_first.solve().unwrap_or_else(|error| panic!("delta={delta:e} A->B: {error:?}"));
            let outcome_b = b_first.solve().unwrap_or_else(|error| panic!("delta={delta:e} B->A: {error:?}"));
            ...
            if delta > 0.0 {
                assert_eq!(outcome_a.phase_control_report.final_phase_set.active_mask(), vec![true, true, false]);
                assert_eq!(outcome_b.phase_control_report.final_phase_set.active_mask(), vec![true, false, true]);
                let losing = outcome_a.acceptance_report.phase_stability[2]
                    .minimum_tpd.expect("losing phase B must have TPD evidence");
                assert!(losing > 0.0);
            ...
            println!("delta_G={delta:.1e} A/B physical_state_invariant=true transitions={}/{}", outcome_a.phase_control_report.transitions.len(), outcome_b.phase_control_report.transitions.len());
        }
        assert_eq!(resolved, 7);
        println!("verdict: ResolvedCompetitionInvariant; exact delta_G=0 classified as non-unique representative");
    }
    ...
    fn i5_controlled_condensed_gibbs_split_measures_observed_tpd_scatter() {
        ...
        for a_before_b in [true, false, true, false, true, false] {
            ...
            let outcome = runner.solve().expect("well-resolved controlled split must solve");
            assert!(outcome.acceptance_report.complementarity.satisfied);
        ...
        println!("controlled Gibbs TPD scatter: delta_G={delta_g:.3e} samples={} min={min:.12e} max={max:.12e} scatter={scatter:.3e}", samples.len());
    }

    #[test]
    fn i5_controlled_condensed_gibbs_split_resolution_matrix() {
        let deltas = [
            1e-6, 1e-7, 1e-8, 1e-9, 1e-10, 1e-11, 1e-12, 1e-13, 1e-14,
            1e-15, 0.0,
        ];
        let base = R * T_K * (ice_boundary_pa(T_K) / P0_PA).ln();
        ...
        println!("requested_delta actual_delta tpd_AB tpd_BA rel_err_AB rel_err_BA winner_AB winner_BA state_delta transitions verdict");
        for requested in deltas {
            ...
            let ab = ab_runner.solve().expect("controlled AB matrix row must terminate");
            let ba = ba_runner.solve().expect("controlled BA matrix row must terminate");
            ...
            let state_delta = ab_moles.iter().zip(ba_physical)
                .map(|(x, y)| (x - y).abs()).fold(0.0, f64::max);
            ...
            let unique = requested > 0.0 && winner_ab && !ab.phase_control_report.final_phase_set.active_mask()[2]
                && winner_ba && !ba.phase_control_report.final_phase_set.active_mask()[1];
            let verdict = if requested == 0.0 {
                ...
                if first_tolerance_limited.is_none() { first_tolerance_limited = Some(requested); }
                "ToleranceLimitedDegeneracy"
            };
            println!("{requested:.3e} {actual:.12e} {:?} {:?} {:?} {:?} {winner_ab} {winner_ba} {state_delta:.3e} {}/{} {verdict}",
                tpd_ab, tpd_ba, rel_ab, rel_ba,
                ab.phase_control_report.transitions.len(), ba.phase_control_report.transitions.len());
            assert!(state_delta.is_finite());
            if requested == 0.0 {
                assert!((ab_moles[0] - ba_physical[0]).abs() < 1e-8);
                assert!((ab_moles[1] - ba_physical[1]).abs() < 1e-8);
                assert!(((ab_moles[2] + ab_moles[3])
                    - (ba_physical[2] + ba_physical[3])).abs() < 1e-8);
                assert!(ab.phase_control_report.transitions.len() <= 2);
                assert!(ba.phase_control_report.transitions.len() <= 2);
            }
            assert!(both_positive || requested == 0.0 || actual == 0.0 || verdict == "ToleranceLimitedDegeneracy");
        }
        ...
        println!("resolution summary: smallest_representable={smallest_representable:?} smallest_quantitative={smallest_quantitative:?} smallest_unique_winner={smallest_unique_winner:?} first_tolerance_limited={first_tolerance_limited:?} floating_point_collapse={floating_point_collapse:?} exact=ExactDegeneracyNonUnique");
    }
}
