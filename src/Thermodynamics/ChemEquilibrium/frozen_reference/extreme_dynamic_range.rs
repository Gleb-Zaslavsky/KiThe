...
mod tests {
    ...
    fn try_solve_with_seed(weights: &[f64], total_moles: f64, seed_moles: &[f64]) -> Result<(Vec<f64>, f64), ReactionExtentError> {
        ...
        let problem = EquilibriumProblem::new(
            ...
            elements,
            gibbs,
            vec![Phase { kind: PhaseActivityModel::IdealGas, species: (0..weights.len()).collect() }],
        ...
        assert_eq!(outcome.phase_control_report.final_phase_set.active_mask(), vec![true]);
    ...
    fn assert_against_oracle(weights: &[f64], actual: &[f64], balance: f64) {
        ...
        for (index, (weight, value)) in weights.iter().zip(actual).enumerate() {
            ...
            assert!(value.is_finite() && *value > 0.0, "X{index} is not finite/positive: {value:e}");
            ...
            println!("X{index} expected={expected:.6e} actual={value:.6e} relative={relative:.3e} log_error={log_error:.3e} mu_minus_mu0={chemical_potential_delta:.3e}");
            // Major and resolved minor species use a scale-aware contract. The
            // log error remains the useful diagnostic for trace species.
            assert!(log_error < 2e-5, "X{index} log-ratio error too large: {log_error:e}");
        }
    ...
    fn i5_extreme_composition_dynamic_range_matrix() {
        ...
        for (label, weights) in cases {
            ...
            println!("case={label} expected_min_fraction={:.3e} actual_min_moles={minimum:.3e} balance={balance:.3e} status=OK", weights.last().unwrap() / weights.iter().sum::<f64>());
            assert_against_oracle(&weights, &actual, balance);
        }
        println!("verdict: ExtremeCompositionDynamicRangeInvariant");
    }
    ...
    fn i5_vanishing_species_resolution_sweep() {
        ...
        for epsilon in epsilons {
            ...
            assert!(value / actual[0] < previous_ratio, "vanishing species amount must decrease monotonically");
            previous_ratio = value / actual[0];
            println!("epsilon={epsilon:.1e} expected_trace={expected:.6e} actual_trace={value:.6e} actual_over_expected={ratio:.6e} log_error={log_error:.3e} balance={balance:.3e} verdict=VanishingSpeciesLogResolved");
        }
    }
    ...
    fn i5_gibbs_weight_representability_matrix() {
        ...
        let ratios: [f64; 11] = [1e-4, 1e-8, 1e-12, 1e-16, 1e-20, 1e-24, 1e-28, 1e-32, 1e-40, 1e-60, 1e-100];
        ...
        for ratio in ratios {
            ...
            println!(
                ...
                if representable { "FloatingPointFixtureRepresentable" } else { "FloatingPointFixtureLimit" }
            );
        ...
        assert!(first_collapsed.is_none(), "the chosen f64 Gibbs matrix must remain representable");
    ...
    fn i5_extreme_composition_is_seed_invariant() {
        ...
        for (index, seed) in seeds.iter().enumerate() {
            let (actual, balance) = try_solve_with_seed(&weights, TOTAL_MOLES, seed)
                .expect("seed route must solve");
            assert!(balance < 1e-10);
            for (value, expected) in actual.iter().zip(&reference) {
                assert!((value - expected).abs() < 1e-10 + 1e-8 * expected.abs());
            }
            println!("seed={index} physical_state_delta={:.3e} balance={balance:.3e} status=OK", actual.iter().zip(&reference).map(|(a, b)| (a-b).abs()).fold(0.0, f64::max));
        }
    ...
    fn i5_extreme_composition_is_species_permutation_invariant() {
        let weights = [1.0, 1e-3, 1e-8, 1e-16, 1e-30];
        let permutations = [[0, 1, 2, 3, 4], [4, 3, 2, 1, 0], [1, 3, 0, 4, 2], [2, 0, 4, 1, 3]];
        let (reference, _) = solve(&weights);
        for permutation in permutations {
            let permuted = permutation.map(|i| weights[i]);
            let (actual, balance) = solve(&permuted);
            let mut canonical = vec![0.0; 5];
            for (position, &original) in permutation.iter().enumerate() { canonical[original] = actual[position]; }
            assert!(balance < 1e-10);
            for (value, expected) in canonical.iter().zip(&reference) {
                assert!((value - expected).abs() < 1e-10 + 1e-8 * expected.abs());
            }
            println!("permutation={permutation:?} physical_state_delta={:.3e} balance={balance:.3e} status=OK", canonical.iter().zip(&reference).map(|(a, b)| (a-b).abs()).fold(0.0, f64::max));
        }
    ...
    fn i5_extreme_composition_preserves_extensive_scaling() {
        ...
        for total in [1e-8, 1.0, 1e8] {
            ...
            let Ok((actual, balance)) = result else {
                all_invariant = false;
                println!("total_moles={total:.3e} verdict=ScaleLimitedCharacterization solver=AllBackendsFailed");
                continue;
            };
            assert!(balance <= total.abs() * 1e-10 + 1e-12);
            let fraction_delta = actual.iter().zip(&reference_fractions).map(|(n, x)| (n/total-x).abs()).fold(0.0, f64::max);
            println!("total_moles={total:.3e} max_fraction_delta={fraction_delta:.3e} balance={balance:.3e}");
            for (value, fraction) in actual.iter().zip(&reference_fractions) {
                assert!((value / total - fraction).abs() < 1e-3, "total={total:e} fraction mismatch");
            }
            let verdict = if fraction_delta < 1e-8 { "DynamicRangeExtensiveScaleInvariant" } else { "ScaleLimitedCharacterization" };
            if verdict != "DynamicRangeExtensiveScaleInvariant" { all_invariant = false; }
            println!("total_moles={total:.3e} max_fraction_delta={fraction_delta:.3e} balance={balance:.3e} verdict={verdict}");
        }
        println!("verdict: {}", if all_invariant { "DynamicRangeExtensiveScaleInvariant" } else { "ScaleLimitedCharacterization" });
    }
}
