...
pub(crate) const CEA_SPECIES: [&str; 40] = [
    "Ar", "CN", "CO", "CO2", "COOH", "H", "H2", "H2O", "H2O2",
    "HCHO,formaldehy", "HCN", "HCO", "HCOOH", "HNC", "HNCO", "HNO",
    "HNO2", "HNO3", "HO2", "N", "N2", "N2H2", "N2O", "N2O3", "N2O4",
    "N3", "N3H", "NCO", "NH", "NH2", "NH2NO2", "NH2OH", "NH3", "NO",
    "NO2", "NO3", "O", "O2", "O3", "OH",
];

pub(crate) const TRACE_ONLY_SPECIES: [&str; 39] = [
    "(HCOOH)2", "C", "C10H8,naphthale", "C2", "C2H", "C2H2,acetylene",
    "C2H2,vinylidene", "C2H3,vinyl", "C2H4", "C2H4O,ethylen-o", "C2H5",
    "C2H5OH", "C2H6", "C3H3,1-propynl", "C3H3,2-propynl", "C3H6O,acetone",
    "C3H6O,propanal", "C3H6O,propylox", "C4H2,butadiyne", "C4H6,1butyne",
    "C4H6,2butyne", "C6H14,n-hexane", "C7H16,2-methylh", "CH", "CH2",
    "CH2CO,ketene", "CH2OH", "CH3", "CH3CHO,ethanal", "CH3CN",
    "CH3CO,acetyl", "CH3COOH", "CH3N2CH3", "CH3O", "CH3O2CH3", "CH3OCH3",
    "CH3OH", "CH3OOH", "CH4",
];
...
pub(crate) fn reconstruct_feed_with_fuel_molar_masses(
    ...
    const AIR_PER_NOMINAL_MOLE: [(&str, f64); 4] = [
        ("N", 1.561680), ("O", 0.419590), ("Ar", 0.009365), ("C", 0.000319),
    ];
    ...
    let air_molar_mass_kg_mol = AIR_PER_NOMINAL_MOLE.iter()
        .map(|(element, count)| {
            let atomic_mass = ATOMIC_MASS.iter().find(|(name, _)| name == element)
                .map(|(_, value)| *value).expect("CEA Air element must have an atomic mass");
            count * atomic_mass
        }).sum::<f64>() / 1000.0;
...
const SCHEMA: [ReferenceColumnSchema; 5] = [
    ReferenceColumnSchema { column: "problem_type", unit: "text" },
    ReferenceColumnSchema { column: "pressure_pa", unit: "Pa" },
    ReferenceColumnSchema { column: "equilibrium_temperature_k", unit: "K" },
    ReferenceColumnSchema { column: "source_specific_enthalpy_j_kg", unit: "J/kg" },
    ReferenceColumnSchema { column: "species_mole_fractions", unit: "mole fraction" },
];

impl FrozenReferenceRow for NasaCeaRp1311Example3Reference {
    fn schema() -> &'static [ReferenceColumnSchema] { &SCHEMA }

    fn validate_rows(dataset_id: &str, rows: &[Self]) -> Result<(), FrozenReferenceError> {
        if rows.len() != 3 {
            return Err(FrozenReferenceError::InvalidRows {
                dataset_id: dataset_id.to_owned(), row: None, field: "rows".to_owned(),
        ...
        for (index, row) in rows.iter().enumerate() {
            if row.problem_type != "HP" {
                return Err(FrozenReferenceError::InvalidRows {
                    dataset_id: dataset_id.to_owned(), row: Some(index), field: "problem_type".to_owned(),
                    message: format!("expected HP, got {}", row.problem_type),
                });
            }
            if !row.pressure_pa.is_finite() || row.pressure_pa <= 0.0
                || !row.equilibrium_temperature_k.is_finite()
                || !row.source_specific_enthalpy_j_kg.is_finite()
            {
                return Err(FrozenReferenceError::InvalidRows {
                    dataset_id: dataset_id.to_owned(), row: Some(index), field: "scalar".to_owned(),
                    message: "pressure, temperature and enthalpy must be finite".to_owned(),
                });
            }
            if row.species_mole_fractions.len() != CEA_SPECIES.len() {
                return Err(FrozenReferenceError::InvalidRows {
                    dataset_id: dataset_id.to_owned(), row: Some(index), field: "species_mole_fractions".to_owned(),
                    message: format!("expected {} published species", CEA_SPECIES.len()),
                });
            }
            for (position, (expected, value)) in CEA_SPECIES.iter()
                .zip(row.species_mole_fractions.iter()).enumerate()
            {
                if value.species != *expected || !value.mole_fraction.is_finite()
                    || value.mole_fraction < 0.0 || value.mole_fraction > 1.0
                {
                    return Err(FrozenReferenceError::InvalidRows {
                        dataset_id: dataset_id.to_owned(), row: Some(index),
...
pub(crate) fn load_dataset() -> Result<FrozenReferenceDataset<NasaCeaRp1311Example3Reference>, String> {
...
mod tests {
    ...
    use crate::Thermodynamics::ChemEquilibrium::equilibrium_ph_workflow::{
        solve_resolved_ph, ResolvedPhaseEnthalpyRequest,
    };
    ...
    fn local_liquid_fuel_molar_masses_kg_mol(
        repository: Arc<ThermoRepository>,
    ) -> (f64, f64) {
        let spec = SubstanceSystemSpecBuilder::new(SubstancesContainer::MultiPhase(
            HashMap::from([
        ...
        let resolved = SubstanceSystemFactory::resolve_phase_system_with_repository(spec, repository)
            .expect("local liquid fuel records must resolve offline");
        assert!(!resolved.report().nist_fallback_enabled());
        let (_, masses_g_mol, _) = element_composition_and_molar_mass(
            resolved.phase_data(), resolved.layout(), None,
        )
    ...
    fn rp1311_example3_source_dataset_has_complete_semantic_universe() {
        ...
        assert!(dataset.rows().iter().all(|row| {
            row.species_mole_fractions.iter().all(|value| value.mole_fraction >= 0.0)
        }));
        println!("RP-1311 Example 3 preflight: rows=3 non_trace_species=40 trace_only_species=39 verdict=ExternalFixtureSourceComplete");
    }
    ...
    fn rp1311_example3_feed_reconstruction_preserves_source_conventions() {
        ...
        assert_eq!(feed.air_mass_kg / feed.fuel_mass_kg, OXIDIZER_FUEL_MASS_RATIO);
        ...
        for element in ["C", "H", "O", "N", "Ar"] {
            assert!(feed.elemental_moles.get(element).copied().unwrap_or_default() > 0.0);
        }
        println!(
            "RP-1311 feed preflight: n_C7H8={:.9e} mol n_C8H18={:.9e} mol nominal_Air={:.9e} mol total_mass={:.6} kg O/F={:.6} H_target={:.6e} J elements={:?}",
            feed.c7h8_moles, feed.c8h18_moles, feed.nominal_air_moles,
            feed.total_mass_kg, feed.air_mass_kg / feed.fuel_mass_kg,
            feed.target_enthalpy_j, feed.elemental_moles
        );
    }
    ...
    fn rp1311_example3_local_liquid_mass_audit_preserves_one_kg_fuel_basis() {
        ...
        let reconstructed_fuel_mass = feed.c7h8_moles * c7_mass_kg_mol
            + feed.c8h18_moles * c8_mass_kg_mol;
        ...
        println!(
            ...
            feed.c7h8_moles, feed.c8h18_moles, reconstructed_fuel_mass,
            feed.air_mass_kg, reconstructed_fuel_mass + feed.air_mass_kg,
        );
    }
    ...
    fn rp1311_example3_liquid_fuel_enthalpy_preflight_is_offline_and_explicit() {
        ...
        let (_, molar_masses, _) = element_composition_and_molar_mass(
            resolved.phase_data(), resolved.layout(), None,
        )
        ...
        println!(
            ...
            bounds.lower(), bounds.upper(), enthalpies[0], enthalpies[1], fuel_enthalpy_j
        );
        println!(
    ...
    fn rp1311_example3_cea_air_reactant_enthalpy_characterization() {
        let air_spec = SubstanceSystemSpecBuilder::new(SubstancesContainer::MultiPhase(
            HashMap::from([(
                "air_source_reconstruction".to_owned(),
                CEA_AIR_SPECIES.iter().map(|(name, _)| (*name).to_owned()).collect(),
            )]),
        ))
        .with_phase_natures(Some(HashMap::from([(
            "air_source_reconstruction".to_owned(), Phases::Gas,
        )])))
        ...
        let (_, air_molar_masses, _) = element_composition_and_molar_mass(
            resolved_air.phase_data(), resolved_air.layout(), None,
        )
        ...
        let (_, fuel_molar_masses, _) = element_composition_and_molar_mass(
            resolved_fuel.phase_data(), resolved_fuel.layout(), None,
        )
        ...
        println!("  Air fractions: N2={:.6} O2={:.6} Ar={:.6} CO2={:.6}", CEA_AIR_SPECIES[0].1, CEA_AIR_SPECIES[1].1, CEA_AIR_SPECIES[2].1, CEA_AIR_SPECIES[3].1);
        println!("  Air molar mass={:.9e} kg/mol nominal_air_moles={:.9e}", air_molar_mass_kg_mol, nominal_air_moles);
        println!("  h_N2={:.9e} h_O2={:.9e} h_Ar={:.9e} h_CO2={:.9e} J/mol", air_enthalpies[0], air_enthalpies[1], air_enthalpies[2], air_enthalpies[3]);
        println!("  h_air_molar={:.9e} J/mol H_air={:.9e} J H_fuel={:.9e} J", h_air_molar, h_air, h_fuel);
        println!("  H_local={:.9e} J H_CEA={:.9e} J h_local={:.9e} J/kg h_CEA={:.9e} J/kg delta={:.9e} J/kg relative={:.9e} verdict={}", h_local, h_cea, specific_h_local, SOURCE_SPECIFIC_ENTHALPY_J_KG, specific_h_local - SOURCE_SPECIFIC_ENTHALPY_J_KG, relative_delta, verdict);
    ...
    fn i5_nasa_cea_rp1311_example3_production_ph_comparison() {
        ...
        let spec = SubstanceSystemSpecBuilder::new(SubstancesContainer::MultiPhase(
            HashMap::from([("gas".to_owned(), gas_species)]),
        ))
        ...
        let air_molar_mass = CEA_AIR_SPECIES.iter()
        ...
        let reconstructed_fuel_mass = c7_moles * c7h8_molar_mass_kg_mol
            + c8_moles * c8h18_molar_mass_kg_mol;
        ...
        assert!((reconstructed_fuel_mass + reconstructed_air_mass - feed.total_mass_kg).abs() < 1.0e-10);
        ...
        let entries = CEA_AIR_SPECIES.iter()
            .map(|(name, fraction)| (
                PhaseComponentId::new(PhaseId::new(Some("gas".to_owned())), *name),
                nominal_air_moles * fraction
                    - if *name == "O2" { oxygen_adjustment_moles } else { 0.0 },
            ))
            .chain([
                (PhaseComponentId::new(PhaseId::new(Some("gas".to_owned())), "CO"), carbon_monoxide_moles),
                (PhaseComponentId::new(PhaseId::new(Some("gas".to_owned())), "H2"), hydrogen_moles),
            ])
            .collect();
        ...
        let seed_elements = [
            ...
            ("O", 2.0 * (air_o2 - oxygen_adjustment_moles) + 2.0 * air_co2 + carbon_monoxide_moles),
            ("N", 2.0 * air_n2),
            ("Ar", air_ar),
        ];
        for (element, seed_total) in seed_elements {
            let source_total = *feed.elemental_moles
                .get(element)
                .expect("reconstructed source feed must contain every seed element");
            assert!((seed_total - source_total).abs() < 1.0e-8 * source_total.abs().max(1.0),
        ...
        println!(
            ...
            reconstructed_fuel_mass + reconstructed_air_mass, feed.target_enthalpy_j,
        );
        println!("seed element totals: {:?}", seed_elements);

        println!("RP-1311 Example 3 production P,H comparison");
        println!("pressure_Pa | P/P0 | CEA_T_K | KiThe_T_K | delta_T_K | max_species_rel | residual | balance | backend/iterations | provenance");
        for reference in dataset.rows() {
            ...
            let max_species_rel = CEA_SPECIES.iter()
                .zip(reference.species_mole_fractions.iter())
                .map(|(name, external)| {
                    let component = PhaseComponentId::new(PhaseId::new(Some("gas".to_owned())), *name);
                    let actual = solution.equilibrium().mole_fraction_for(&component)
                        .expect("published solution must contain every CEA gas species");
                    (actual - external.mole_fraction).abs()
                        / external.mole_fraction.max(1.0e-12)
                })
                .fold(0.0_f64, f64::max);
            let max_major_rel = CEA_SPECIES.iter()
                .zip(reference.species_mole_fractions.iter())
                ...
                .fold(0.0_f64, f64::max);
            let max_minor_trace_log10 = CEA_SPECIES.iter()
                .zip(reference.species_mole_fractions.iter())
                .filter(|(_, external)| {
                ...
                .map(f64::abs)
                .fold(0.0_f64, f64::max);
            let fraction_sum: f64 = CEA_SPECIES.iter()
                .map(|name| solution.equilibrium().mole_fraction_for(
                    &PhaseComponentId::new(PhaseId::new(Some("gas".to_owned())), *name),
                ).unwrap_or_default())
                .sum();
            ...
            println!(
                ...
                reference.pressure_pa, pressure_ratio, reference.equilibrium_temperature_k, temperature,
                delta_t, max_species_rel, max_major_rel, max_minor_trace_log10, validation.residual_l2_norm,
            ...
            assert!(validation.residual_l2_norm.is_finite() && validation.residual_l2_norm < 1.0e-5);
            assert!(validation.max_abs_element_balance_error.is_finite() && validation.max_abs_element_balance_error < 1.0e-5);
        }
    }
}
