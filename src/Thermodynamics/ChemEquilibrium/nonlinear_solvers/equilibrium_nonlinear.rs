...
/// One rejected physical-temperature start in bounded monolithic P,H
/// recovery. The original typed cause remains available for backend-level
/// diagnostics.
#[derive(Debug)]
pub struct PhMonolithicSeedFailure {
    temperature_seed_k: f64,
    cause: Box<ReactionExtentError>,
}

impl PhMonolithicSeedFailure {
    pub(crate) fn new(temperature_seed_k: f64, cause: ReactionExtentError) -> Self {
        Self {
            temperature_seed_k,
            cause: Box::new(cause),
        }
    ...
    /// Physical temperature used to construct this fresh nonlinear start.
    pub fn temperature_seed_k(&self) -> f64 {
        self.temperature_seed_k
    }

    /// Typed backend/formulation cause for this rejected start.
    pub fn cause(&self) -> &ReactionExtentError {
        &self.cause
    }

    pub(crate) fn into_cause(self) -> ReactionExtentError {
        *self.cause
    }
}
...
pub enum ReactionExtentError {
    ...
    /// A physical solve failed numerically and the explicit extensive-
    /// normalization recovery could not complete normalized discovery or the
    /// audited physical publication stage. Both causes remain available for
    /// release diagnostics.
    ExtensiveNormalizationRecoveryFailed {
        /// Recovery stage that failed after the original physical rejection.
        stage: &'static str,
        /// Original failure in physical extensive coordinates.
        physical: Box<ReactionExtentError>,
        /// Failure produced by normalized discovery or physical publication.
        recovery: Box<ReactionExtentError>,
    },
    ...
    /// Every deterministic physical-temperature start for the monolithic P,H
    /// formulation was rejected. Each start owns its original backend trace.
    PhMonolithicSeedRecoveryFailed {
        /// Ordered fresh temperature starts and their typed causes.
        attempts: Vec<PhMonolithicSeedFailure>,
    },
    /// A bounded monolithic P,H all-active probe solved numerically, but its
    /// TPD evidence did not authorize publishing the probed phase set.
    ///
    /// This is not malformed input and not an accepted physical candidate.
    /// The independently formulated nested-temperature route may therefore
    /// recover when `P,H Auto` is selected.
    PhMonolithicPhaseProbeRejected {
        /// Auditable reason why the probe could not become a lifecycle state.
        message: String,
    },
...
impl fmt::Display for ReactionExtentError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::ExtensiveNormalizationRecoveryFailed {
                stage,
                physical,
                recovery,
            } => write!(
                f,
                "extensive-normalization recovery failed during {stage}; physical attempt: {physical}; recovery attempt: {recovery}"
            ),
            ...
            Self::PhMonolithicSeedRecoveryFailed { attempts } => write!(
                f,
                "monolithic P,H temperature-seed recovery failed after {} attempt(s)",
                attempts.len()
            ),
            Self::PhMonolithicPhaseProbeRejected { message } => {
                write!(f, "monolithic P,H phase probe was rejected: {message}")
            }
...
impl Error for ReactionExtentError {
    fn source(&self) -> Option<&(dyn Error + 'static)> {
        match self {
            ...
            Self::ExtensiveNormalizationRecoveryFailed { recovery, .. } => Some(recovery.as_ref()),
            ...
            Self::PhMonolithicSeedRecoveryFailed { attempts } => {
                attempts.last().map(|attempt| attempt.cause() as _)
            }
            _ => None,
        }
    }
...
impl ReactionExtentError {
    ...
    pub fn kind(&self) -> ReactionExtentErrorKind {
        match self {
            ...
            Self::ExtensiveNormalizationRecoveryFailed { recovery, .. } => recovery.kind(),
            ...
            Self::PhMonolithicSeedRecoveryFailed { .. } => {
                ReactionExtentErrorKind::AllBackendsFailed
            }
            Self::PhMonolithicPhaseProbeRejected { .. } => {
                ReactionExtentErrorKind::InvalidCandidate
            }
        }
    }

    /// Number of concrete nonlinear backend attempts retained by this error.
    ///
    /// Wrapper errors are traversed recursively so P,H seed recovery, `Auto`
    /// fallback, and range failures can enforce one global work budget.
    pub fn started_backend_attempts(&self) -> usize {
        match self {
            Self::AllBackendsFailed { attempts } | Self::CascadeAborted { attempts, .. } => {
                attempts
                    .iter()
                    .filter(|attempt| {
                        !matches!(attempt.outcome, SolverAttemptOutcome::Skipped { .. })
                    })
                    .count()
            }
            Self::TemperatureTrialFailed { cause, .. }
            | Self::TemperatureRangePointFailed { cause, .. } => cause.started_backend_attempts(),
            Self::ExtensiveNormalizationRecoveryFailed {
                physical, recovery, ..
            } => physical
                .started_backend_attempts()
                .saturating_add(recovery.started_backend_attempts()),
            Self::PhAutoFallbackFailed { monolithic, nested } => monolithic
                .started_backend_attempts()
                .saturating_add(nested.started_backend_attempts()),
            Self::PhMonolithicSeedRecoveryFailed { attempts } => attempts
                .iter()
                .map(|attempt| attempt.cause().started_backend_attempts())
                .sum(),
            _ => 0,
        }
    }

    /// Aggregate nonlinear iterations retained by this error's backend trace.
    pub fn nonlinear_iterations(&self) -> usize {
        match self {
            ...
            Self::TemperatureTrialFailed { cause, .. }
            | Self::TemperatureRangePointFailed { cause, .. } => cause.nonlinear_iterations(),
            Self::ExtensiveNormalizationRecoveryFailed {
                physical, recovery, ..
            } => physical
                .nonlinear_iterations()
                .saturating_add(recovery.nonlinear_iterations()),
            Self::PhAutoFallbackFailed { monolithic, nested } => monolithic
                .nonlinear_iterations()
                .saturating_add(nested.nonlinear_iterations()),
            Self::PhMonolithicSeedRecoveryFailed { attempts } => attempts
                .iter()
                .map(|attempt| attempt.cause().nonlinear_iterations())
                .sum(),
            _ => 0,
        }
    }
    ...
    pub fn is_retryable_formulation_failure(&self) -> bool {
        match self {
            ...
            Self::PhMonolithicPhaseProbeRejected { .. } => true,
    ...
    pub fn is_non_retryable_input_error(&self) -> bool {
        if matches!(self, Self::PhMonolithicPhaseProbeRejected { .. }) {
            return false;
        }
        matches!(
            self.kind(),
...
mod error_contract_tests {
    ...
    #[test]
    fn rejected_monolithic_phase_probe_allows_only_formulation_fallback() {
        let error = ReactionExtentError::PhMonolithicPhaseProbeRejected {
            message: "TPD did not authorize the probed phase".to_string(),
        };

        assert_eq!(error.kind(), ReactionExtentErrorKind::InvalidCandidate);
        assert!(!error.is_retryable_backend_failure());
        assert!(error.is_retryable_formulation_failure());
        assert!(!error.is_non_retryable_input_error());
        assert!(error.to_string().contains("phase probe was rejected"));
    }

    #[test]
    fn aggregate_ph_errors_report_started_backend_work_recursively() {
        let backend = SolverBackend::Legacy(Solvers::NR);
        let failed_seed = |temperature| {
            PhMonolithicSeedFailure::new(
                temperature,
        ...
        let monolithic = ReactionExtentError::PhMonolithicSeedRecoveryFailed {
            attempts: vec![failed_seed(600.0), failed_seed(1_000.0)],
        };
        assert_eq!(monolithic.started_backend_attempts(), 2);
        assert_eq!(monolithic.nonlinear_iterations(), 0);

        let auto = ReactionExtentError::PhAutoFallbackFailed {
            monolithic: Box::new(monolithic),
            nested: Box::new(ReactionExtentError::AllBackendsFailed {
                attempts: vec![SolverAttemptReport {
                    backend,
                    outcome: SolverAttemptOutcome::Failed {
                        kind: SolverAttemptFailureKind::Solver,
                        reason: "nested iteration limit".to_string(),
                    },
                    metrics: None,
                }],
            }),
        };
        assert_eq!(auto.started_backend_attempts(), 3);
    }
