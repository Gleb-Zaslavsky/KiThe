...
const ELEMENTS: &[Element] = &[
    Element { name: "H", atomic_mass: 1.008 },
    Element { name: "He", atomic_mass: 4.0026 },
    Element { name: "Li", atomic_mass: 6.94 },
    Element { name: "Be", atomic_mass: 9.0122 },
    Element { name: "B", atomic_mass: 10.81 },
    Element { name: "C", atomic_mass: 12.011 },
    Element { name: "N", atomic_mass: 14.007 },
    Element { name: "O", atomic_mass: 15.999 },
    Element { name: "F", atomic_mass: 18.998 },
    Element { name: "Ne", atomic_mass: 20.18 },
    Element { name: "Na", atomic_mass: 22.99 },
    Element { name: "Mg", atomic_mass: 24.305 },
    Element { name: "Al", atomic_mass: 26.98 },
    Element { name: "Si", atomic_mass: 28.085 },
    Element { name: "P", atomic_mass: 30.974 },
    Element { name: "S", atomic_mass: 32.065 },
    Element { name: "Cl", atomic_mass: 35.45 },
    Element { name: "Ar", atomic_mass: 39.948 },
    Element { name: "K", atomic_mass: 39.102 },
    Element { name: "Ca", atomic_mass: 40.08 },
    Element { name: "Sc", atomic_mass: 44.9559 },
    Element { name: "Ti", atomic_mass: 47.867 },
    Element { name: "V", atomic_mass: 50.9415 },
    Element { name: "Cr", atomic_mass: 51.9961 },
    Element { name: "Mn", atomic_mass: 54.938 },
    Element { name: "Fe", atomic_mass: 55.845 },
    Element { name: "Co", atomic_mass: 58.933 },
    Element { name: "Ni", atomic_mass: 58.69 },
    Element { name: "Cu", atomic_mass: 63.546 },
    Element { name: "Zn", atomic_mass: 65.38 },
    Element { name: "Ga", atomic_mass: 69.723 },
    Element { name: "Ge", atomic_mass: 72.64 },
    Element { name: "As", atomic_mass: 74.9216 },
    Element { name: "Se", atomic_mass: 78.96 },
    Element { name: "Br", atomic_mass: 79.904 },
    Element { name: "Kr", atomic_mass: 83.798 },
    Element { name: "Rb", atomic_mass: 85.4678 },
    Element { name: "Sr", atomic_mass: 87.62 },
    Element { name: "Y", atomic_mass: 88.9059 },
    Element { name: "Zr", atomic_mass: 91.224 },
    Element { name: "Nb", atomic_mass: 92.9064 },
    Element { name: "Mo", atomic_mass: 95.94 },
    Element { name: "Tc", atomic_mass: 98.0 },
    Element { name: "Ru", atomic_mass: 101.07 },
...
// Function to parse a chemical formula and return a HashMap of elements and their counts
pub fn parse_formula(formula: String) -> HashMap<String, usize> {
    ...
    while i < formula.len() {
        ...
        if formula.chars().nth(i).unwrap() =='('{ 
            ...
            println!("length of formula after deleting brackets {}", formula.len());
         
        }
     //  println!("start bracket position {}, end bracket position {}", start_bracket, end_bracket);
        // vector of element names
        let element_names = ELEMENTS.iter().map(|element| element.name).collect::<Vec<_>>();
        ...
        if  formula.chars().nth(i).unwrap().is_uppercase() && i < formula.len() {
            println!("Find uppercase letter: {},  move to the next position {}", formula.chars().nth(i).unwrap(), i+1);
            i += 1;
            ...
            if let Some(c) = formula.chars().nth(i) {
                if c.is_lowercase() {
                    println!("Find lowercase letter: {}, move to the next position {}", formula.chars().nth(i).unwrap(), i+1);
                    i += 1;
        
                } else if  c.is_uppercase() &&
                 !element_names.contains(&c.to_string().as_str())

                  {
                    ...
                    formula.replace_range(i..i+1, formula.chars().nth(i).unwrap().to_lowercase().to_string().as_str());
                    i += 1;
                }
            } else {
      ...
      //let element = &formula[start..i];
        ...
        if element.is_empty() {panic!("element is empty");}
 
        let mut count = 1;
        
        fn after_bracket_stoichio(end_bracket: usize, formula: String) -> usize {
            let mut end_of_stoichio_after_bracket = end_bracket + 1;
            while end_of_stoichio_after_bracket < formula.len() && formula.chars().nth( end_of_stoichio_after_bracket  ).unwrap().is_digit(10) {
                           
                           end_of_stoichio_after_bracket += 1;
                       }
           let stoichio: usize = formula[end_bracket..  end_of_stoichio_after_bracket].parse().unwrap();
           stoichio      
        }
        // if we have no elements in brackets or current element is out of brackets
        if ( start_bracket==0 && end_bracket==0 && i < formula.len() ) || i<start_bracket || i > end_bracket {
        ...
        // if we have elements in brackets and current element is in brackets
        else if start_bracket!=0 && end_bracket!=0 && i < formula.len() && i > start_bracket && i <= end_bracket {
            ...
            println!("bracket positions {}, {}, current position: {}", start_bracket, end_bracket, i);
            ...
            // found stoichio inside brackets
                ...
                println!("find digit: {} inside brackets", formula.chars().nth(i).unwrap());
                println!("it is a digit?: {}", i_is_digit);  
                println!("bracket positions {}, {}, current position: {}", start_bracket, end_bracket, i);
                println!("element {} inside brackets", element);
                let stoichio_after_bracket: usize = after_bracket_stoichio(end_bracket, formula.clone());
                ...
                while end < formula.len() && formula.chars().nth(end).unwrap().is_digit(10) && end<end_bracket {
                    println!("end: {}", end);
                    end += 1;}
                ...
                let stoichio_after_element: usize = if i<end_bracket { formula[i..end].parse().unwrap()
                } else {
        ...
        *counts.entry(element.to_string()).or_insert(0) += count;
        println!("hashmap of elements updated: {:?} added {} atoms {} at position {}", &counts, count, element, i);
        if i >= formula.len() {
            println!("end of parsing formula {}", &initial_formula);
            break
        } else {
            println!("position: {} while length of formula: {} - go to next loop", i, formula.len());
        }
     }//  end of while loop
    counts
...
pub fn calculate_molar_mass(formula: String) -> (f64, HashMap<String, usize>) {
...
pub fn calculate_molar_mass_of_vector_of_elements(vec_of_formulae: Vec<&str>) -> Vec<f64> {
    let mut molar_masses = Vec::new();
    for formula in vec_of_formulae {
        let counts = parse_formula(formula.to_string());
        let mut molar_mass = 0.0;
...
mod tests {
    ...
    fn test_parse_formula() {
        ...
        let expected_counts = HashMap::from([("C".to_string(), 6), ("H".to_string(), 8), ("O".to_string(), 6)]);
        assert_eq!(parse_formula(formula), expected_counts);
         
        let formula = "Na(NO3)2".to_string();
        let expected_counts = HashMap::from([("Na".to_string(), 1), ("N".to_string(), 2),("O".to_string(), 6) ]);
        assert_eq!(parse_formula(formula), expected_counts);
    ...
    fn test_calculate_molar_mass_of_vector_of_substances() {
        ...
        let calculated_molar_masses = calculate_molar_mass_of_vector_of_elements(vec_of_formulae);
